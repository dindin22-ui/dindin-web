...
if($asap != '3'){
    Yii::app()->functions->updateOption("order_asap",2 , $merchant_id);
}
?>
<div class="uk-form-row">
  <label class="uk-form-label"><?php echo Yii::t("default","Enabled Order ASAP Option")?></label>  
  <?php 
  echo CHtml::checkBox('order_asap',
   Yii::app()->functions->getOption('order_asap',$merchant_id)==2?true:false
   ,array(
   'class'=>"icheck",
   'value'=>2
  ))
...
if($order_special_notes != '3'){
    Yii::app()->functions->updateOption("order_special_notes",'2' , $merchant_id);
}
?>
<div class="uk-form-row">
  <label class="uk-form-label"><?php echo Yii::t("default","Enabled Special Notes")?></label>  
  <?php 
  echo CHtml::checkBox('order_special_notes',
   Yii::app()->functions->getOption('order_special_notes',$merchant_id)=='2'?true:false
   ,array(
   'class'=>"icheck",
   'value'=>'2'
  ))
...
<?php foreach ($m_holiday as $m_h):
  $m_h_f = $m_holiday_from[$i];
  $m_h_t = $m_holiday_to[$i];
  ?>
<div class="holiday_row">
<?php echo CHtml::textField('merchant_holiday[]',$m_h,array(
  'class'=>"j_date_normal small_date holiday_date"
));?> From: <?php echo CHtml::textField("stores_time_from[]",$m_h_f
,array('placeholder'=>Yii::t("default","From"),'id'=>"from_time_".$i,'class'=>"timepick"));?> To: <?php echo CHtml::textField("stores_time_to[]",$m_h_t
,array('placeholder'=>Yii::t("default","To"),'id'=>"to_time_".$i,'class'=>"timepick"));?>
...
<div class="holiday_row" style="display:none;">

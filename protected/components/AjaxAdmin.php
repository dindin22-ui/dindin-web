...
if (!class_exists('AjaxAdmin')) {
    ...
    class AjaxAdmin extends DbExt {
        ...
        public function addMerchant() {
            ...
            $store_id = isset($this->data['uber_eats_store_id']) ? $this->data['uber_eats_store_id'] : "";
            $uber_eats_status = isset($this->data['uber_eats_status']) ? $this->data['uber_eats_status'] : "PAUSED";
            if($store_id !== ""){
                $store_write_status_token = FunctionsV3::getUberEatsToken('eats.store.status.write');
                $update_status = FunctionsV3::updateRestaurantStatus($store_id,$store_write_status_token,$uber_eats_status);
            }
            ...
            $params['uber_eats_key'] = $this->data['uber_eats_key'];
            $params['uber_eats_status'] = $uber_eats_status;
            $params['uber_eats_secret'] = $this->data['uber_eats_secret'];
            $params['uber_eats_store_id'] = $this->data['uber_eats_store_id'];
            ...
            //  print_r($this->data['any_fee']); exit('inn');   
            $params['any_fee'] = isset($this->data['any_fee']) ? $this->data['any_fee'] : '0';
            $params['doordash_fee'] = isset($this->data['doordash_fee']) ? $this->data['doordash_fee'] : '0';

            if ($this->data['merchant_type'] == 1) {
                $params['is_commission'] = 1;
            } else
                $params['is_commission'] = 2;
            ...
            if (isset($this->data['access'])) {
                ...
                $params['user_access'] = json_encode(array());

            //  print_r($params); exit('inn'); 
            ...
            if (empty($this->data['id'])) {
                if ($this->insertData("{{merchant}}", $params)) {
                    ...
                    Yii::app()->functions->updateOption("merchant_enabled_tip",
                        isset($this->data['merchant_enabled_tip']) ? $this->data['merchant_enabled_tip'] : 2
                        , $mtid);
                        
                    Yii::app()->functions->updateOption("display_default_settings",
                        isset($this->data['display_default_settings']) ? $this->data['display_default_settings'] : 0
                        , $mtid);                        

                    Yii::app()->functions->updateOption("merchant_auto_print",
        ...
        public function merchantListx() {
            ...
            if ($res = $this->rst($stmt)) {
                foreach ($res as $val) {
                    ...
                    ';
                    $val['app_status'] = CHtml::checkBox('status',
        ...
        public function resumeTakingOrder() {
    		...
    		if ( $DbExt->updateData('{{mobile_device_merchant}}',$params,'merchant_id',$mtid)){
                  
    			if (isset($store_start_date)){
    				Yii::app()->functions->updateOption("store_start_date",'',$mtid);  
    			}
    			Yii::app()->functions->updateOption("end_time",'',$mtid); 
    
    			if (isset($store_close_date)){
    				Yii::app()->functions->updateOption("store_close_date",'',$mtid);  
    			}
    			$this->code = 1;
    			$this->msg = Yii::t("default", "Order Resumed Successfully");
                return;				
    		}else{
    		    $this->msg = Yii::t("default", "Something went wrong. Please try again");
                return;	
    		}
        }
        ...
        public function merchantLogin() {
            ...
            if ($res = $this->rst($stmt)) {
                if ($res[0]['status'] == "active" || $res[0]['status'] == "expired") {
                    ...
                    $this->msg = Yii::t("default", "Login Failed. You account status is [status]", array(
        ...
        public function addAddOnCategory() {
            ...
            $command = Yii::app()->db->createCommand();
            if (isset($this->data['id']) && is_numeric($this->data['id'])) {
                if (empty($this->data['subcategory_print_name'])) {
                    $subcat_print_name = '';
                } else {
                    $subcat_print_name = addslashes($this->data['subcategory_print_name']);
                }
                $params['subcategory_print_name'] = $subcat_print_name;
                ...
                if (empty($this->data['subcategory_print_name'])) {
                    $subcat_print_name = addslashes($this->data['subcategory_name']);
                } else {
                    $subcat_print_name = addslashes($this->data['subcategory_print_name']);
                }
        ...
        public function merchantSettings() {
            ...
            Yii::app()->functions->updateOption("merchant_enabled_auto_confirm_prep_time",
                    isset($this->data['merchant_enabled_auto_confirm_prep_time']) ? $this->data['merchant_enabled_auto_confirm_prep_time'] : ''
                    , $merchant_id);
            
            Yii::app()->functions->updateOption("merchant_auto_prep_time",
                    isset($this->data['merchant_auto_prep_time']) ? $this->data['merchant_auto_prep_time'] : ''
                    , $merchant_id);
            ...
            Yii::app()->functions->updateOption("stores_time_to",
                    isset($this->data['stores_time_to']) ? json_encode($this->data['stores_time_to']) : ''
                    , $merchant_id);

            Yii::app()->functions->updateOption("stores_time_from",
                    isset($this->data['stores_time_from']) ? json_encode($this->data['stores_time_from']) : ''
                    , $merchant_id);
            ...
            Yii::app()->functions->updateOption("order_special_notes",
                    isset($this->data['order_special_notes']) ? $this->data['order_special_notes'] : '3'
                    , $merchant_id);
        ...
        public function addToCart() {
            ...
            if (isset($this->data['item_id'])) {
                ...
                if (is_numeric($this->data['row'])) {
                    ...
                    $merchant = FunctionsV3::getMerchantInfo($this->data['merchant_id']);
                    if($merchant['service'] == 8){
...
//return;
        ...
        public function setDeliveryOptions() {
            ...
            if($this->data['delivery_type'] == 'delivery'){
                $_SESSION['kr_delivery_options']['email_address'] = $this->data['email_address'];
                $_SESSION['kr_delivery_options']['first_name'] = $this->data['first_name'];
                $_SESSION['kr_delivery_options']['last_name'] = $this->data['last_name'];
                $_SESSION['kr_delivery_options']['street'] = $this->data['street'];
                $_SESSION['kr_delivery_options']['city'] = $this->data['city'];
                $_SESSION['kr_delivery_options']['state'] = $this->data['state'];
                $_SESSION['kr_delivery_options']['zipcode'] = $this->data['zipcode'];
                $_SESSION['kr_delivery_options']['location_name'] = $this->data['location_name'];
                $_SESSION['kr_delivery_options']['contact_phone'] = $this->data['contact_phone'];
                $_SESSION['kr_delivery_options']['delivery_instruction'] = $this->data['delivery_instruction'];
            }
            ...
            if (isset($_SESSION['kr_client'])){
                $this->details = Yii::app()->createUrl('store/paymentoption');
            }else {
                $this->details = Yii::app()->createUrl('store/guestcheckout');
            }
        }
            ...
            if (is_array($order_item) && count($order_item) >= 1) {
                ...
                if (Yii::app()->functions->code == 1) {
                    ...
                    if (is_array($raw) && count($raw) >= 1) {
                        ...
                        $merchant_info=Yii::app()->functions->getMerchant(isset($params['merchant_id'])?$params['merchant_id']:'');
                        if($merchant_info['service'] == 8 && $params['trans_type'] == 'delivery'){
                            $params['doordash_fee'] = Yii::app()->functions->getMerchantDoordashFee($params['merchant_id']);
                        }
                        /* FIXED ORDER STATUS */
                        ...
                        if (Yii::app()->functions->isMerchantCommission($this->data['merchant_id'])) {
                            ...
                            if ($com = Yii::app()->functions->getMerchantCommission($this->data['merchant_id'])) {
                                ...
                                if ($anyFee) {
                                    ...
                                    $params['total_commission'] = ($com / 100) * $params['total_w_tax'] + $anyFee;
                                } else {
                ...
                public function addToOrder() {
                    if (isset($this->data['order_id'])) {
                        if ($res = Yii::app()->functions->getOrder($this->data['order_id'])) {
                            ...
                            $this->msg = t("Order details not available");
                    } else
                ...
                public function SumOfTotal2() {
                    ...
                    if (isset($_POST['start_date']) && isset($_POST['end_date'])) {
                        if (!empty($_POST['start_date']) && !empty($_POST['end_date'])) {
                            $and = " AND date_created BETWEEN  " . FunctionsV3::q($_POST['start_date'] . " 00:00:00") . " AND 
                        " . FunctionsV3::q($_POST['end_date'] . " 23:59:00") . "
                 ";
                        }
                    }
                    ...
                    $order_status_id = '';
                    $or = '';
                    if (isset($_POST['stats_id'])) {
                        if (is_array($_POST['stats_id']) && count($_POST['stats_id']) >= 1) {
                            foreach ($_POST['stats_id'] as $stats_id) {
                    ...
                    $stmt = "SELECT count(order_id) as total_row_count,SUM(total_w_tax) as total, SUM(packaging) as conveniencefee, SUM(total_commission) as commission, SUM(cart_tip_value) as tips
            from
            {{order}}
            where
            merchant_id= " . FunctionsV3::q($merchant_id) . "AND status NOT IN ('" . initialStatus() . "')" . $and;

                    $restotal = $DbExtt->rst($stmt);
                    
                    $stmt2 = "SELECT count(order_id) as total_row_del
                    from
                    {{order}}
                    where
                    merchant_id= " . FunctionsV3::q($merchant_id) . "AND trans_type = 'delivery' AND status NOT IN ('" . initialStatus() . "')" . $and;

                    $res_del = $DbExtt->rst($stmt2);
                    
                    if ($restotal) {
                        $doordash_fee = Yii::app()->functions->getMerchantDoordashFee($merchant_id)*$res_del[0]['total_row_del'];
                        $any_fee = Yii::app()->functions->getMerchantAnyFee($merchant_id)*$restotal[0]['total_row_count'];
                        $commission_calculated = Yii::app()->functions->getMerchantCommissionCalculated($merchant_id);
                        
    	    			$total_commission_row = ($restotal[0]['total']*$commission_calculated);//+$val['tip_value'];	
    	    			$total_commission_row = $total_commission_row+$doordash_fee+$any_fee;
	    			
                        $restotal[0]['total'] = prettyFormat($restotal[0]['total'], $merchant_id);
                        $restotal[0]['conveniencefee'] = prettyFormat($restotal[0]['conveniencefee'], $merchant_id);
                        $restotal[0]['commission'] = prettyFormat($total_commission_row, $merchant_id);
                        $restotal[0]['totalMinusComm'] = $restotal[0]['total'] - $total_commission_row;
                        $restotal[0]['totalMinusComm'] = prettyFormat($restotal[0]['totalMinusComm'], $merchant_id);
                        $restotal[0]['tips'] = prettyFormat($restotal[0]['tips'], $merchant_id);
                    }


                    echo json_encode($restotal[0]);
                    die();
                    //  return $res[0]; 
                }
                ...
                public function salesReport() {
                 ...
                 ";
                    ...
                    $any_fee = Yii::app()->functions->getMerchantAnyFee($merchant_id);
                    $commission_calculated = Yii::app()->functions->getMerchantCommissionCalculated($merchant_id);
            ...
            ";
                    ...
                    if ($res = $DbExt->rst($stmt)) {
                        foreach ($res as $val) {
                            ...
                            $total_commission = $commission_calculated*$val['total_w_tax'];
                            
                            // $totalMinusFeeComm = $val['total_w_tax'] - $val['packaging'];
                            if($val['status'] == 'Paid'){
                                $total_commission = $total_commission+$any_fee;//+$val['cart_tip_value'];
                                if($doordash_fee !== '' && $val['trans_type'] == "delivery"){
                                    $total_commission = $total_commission+$doordash_fee;
                                }
                            }
...
//                    dump($this->data);
            ...
            <?php if ($res = Yii::app()->functions->getOrderInfo($this->data['id'])): ?>
                ...
                if ($this->data['currentController'] == "admin") {
                    ...
                    echo CHtml::hiddenField('action', 'updateOrder');
                ?>
        ...
        public function recentOrder() {
            ...
            WHERE
            merchant_id= " . FunctionsV3::q($merchant_id) . "
            AND delivery_date like '" . date('Y-m-d') . "%'
            AND status NOT in ('" . initialStatus() . "')
...
//                        $new .= '<span class="tag Paid">Order from GrubHub</span>';
                        $order_items = json_decode($val['delivery_service_order_details'],true);
                        $item = array();
                        ...
                        if ($res = $db_ext->rst($sql)) {
                                ...
                                foreach ($res as $val) 
                                    {                             
                                        ...
                                        $any_fee = Yii::app()->functions->getMerchantAnyFee($merchant_id);
                                        $commission_calculated = Yii::app()->functions->getMerchantCommissionCalculated($merchant_id);
                                        $total_commission_row = $commission_calculated*$val['total_w_tax'];
                                    
                                        // $totalMinusFeeComm = $val['total_w_tax'] - $val['packaging'];
                                        // if($val['status'] == 'Paid'){
                                            $total_commission_row = $total_commission_row+$any_fee;//+$val['cart_tip_value'];
                                            if($doordash_fee !== '' && $val['trans_type'] == "delivery"){
                                                $total_commission_row = $total_commission_row+$doordash_fee;
                                            }
                                        // }             
                                        // echo $total_commission_row."<pre>";
                                        // print_r($val);
                                        $total_commission = $total_commission + $total_commission_row; 
                                        $totalMinusFeeCommRaw =$val['total_w_tax'] - $total_commission_row;                                
            ...
            if (!empty($_SESSION['kr_export_stmt'])) {
                ...
                switch ($this->data['rpt']) {
                    case 'sales-report':
                        if ($res = $db_ext->rst($stmt)) {
                            ...
                            foreach ($res as $val) {
                                ...
                                $totalMinusFeeComm = $val['total_w_tax'] - $total_commission;
                                $item = '';
                                $date = date('m-d-Y G:i:s', strtotime($val['date_created']));
                                $latestdata[] = array(
                                    $val['order_id'],
                    ...
                    case "rptmerchantcommission":
                        // echo $_SESSION['kr_export_stmt'];exit;
                        if ($res = $db_ext->rst($_SESSION['kr_export_stmt'])) {
                            foreach ($res as $val) {
                                $date = prettyDate($val['date_created'], true);
                                $date = Yii::app()->functions->translateDate($date);
                                $merchant_id = $val['merchant_id'];
                                $doordash_fee = Yii::app()->functions->getMerchantDoordashFee($merchant_id)*$val['total_delivery_orders_count'];
                                $any_fee = Yii::app()->functions->getMerchantAnyFee($merchant_id)*$val['total_orders_count'];
                                $commission_calculated = Yii::app()->functions->getMerchantCommissionCalculated($merchant_id);
                                
            	    			$anyFee = Yii::app()->functions->getMerchantAnyFee($val['merchant_id']);
            	    			$total_commission_row = ($val['total_order']*$commission_calculated);//+$val['tip_value'];	
            	    			$total_commission_row = $total_commission_row+$doordash_fee+$any_fee;
            	    			$data[] = array(
                                    $val['merchant_id'],
                                    $val['merchant_name'],
		...
		**/
        ...
        public function adminSettings() {
            ...
            Yii::app()->functions->updateOptionAdmin("drive_api_mode",
                isset($this->data['drive_api_mode']) ? $this->data['drive_api_mode'] : 'sandbox');

            Yii::app()->functions->updateOptionAdmin("drive_api_key_sandbox",
                isset($this->data['drive_api_key_sandbox']) ? $this->data['drive_api_key_sandbox'] : '5c30ecce61eba36bf6e6f2de4c4fa625a598757b');

            Yii::app()->functions->updateOptionAdmin("drive_api_key_live",
                isset($this->data['drive_api_key_live']) ? $this->data['drive_api_key_live'] : '');

            Yii::app()->functions->updateOptionAdmin("notify_email",
                isset($this->data['notify_email']) ? $this->data['notify_email'] : '');
        ...
        public function merchantAppSetReady() {
            if (isset($this->data['app_status'])) {
                $mtid = $this->data['merchantId'];
                $params = array(
                ...
                if ($this->updateData("{{merchant}}", $params, 'merchant_id', $mtid)) {
                    ...
                    if ($this->data['app_status'] == 1) {
                        $this->msg = Yii::t("default", "Successful Merchant is now visible on app.");
                    } else
                        $this->msg = Yii::t("default", "Successful");
                } else
                    $this->msg = Yii::t("default", "ERROR: cannot update status");
        ...
        public function clientEmailNotification() {
            ...
            if (isset($this->data['app_status'])) {
               ...
               // print_r($params); exit('ppp');
                if ($this->updateData("{{client}}", $params, 'client_id', $clientid)) {
                    $this->code = 1;
                    if ($this->data['app_status'] == 1) {
                        $this->msg = Yii::t("default", " Email notification subscribe Successfully.");
                    } else
                        $this->msg = Yii::t("default", "Email notification unsubscribe Successful");
                } else
                    $this->msg = Yii::t("default", "ERROR: cannot update status");
            } else
                $this->msg = Yii::t("default", "Missing parameters");
                    ...
                    switch ($social_info[0]['status']) {
                        case "pending":
                            if (strlen($social_info[0]['email_verification_code']) > 2) {
                                ...
                                $this->msg = t("Your account status is pending");
                            break;
...
//                        $this->msg = Yii::t("default", "Sorry but you have already use this voucher code");
        ...
        public function customerList() {
            ...
            $aColumns = array(
                'a.client_id', 'a.first_name', 'a.email_address', 'a.contact_phone', 'a.social_strategy', 'a.status', 'mm.restaurant_name'
            );
            ...
            $sTable = "{{client}} a LEFT JOIN mt_order mo ON a.client_id = mo.client_id
            left join mt_merchant mm ON mo.merchant_id = mm.merchant_id 
            left join mt_order mo1 ON (mo.client_id=mo1.client_id AND mo.order_id<mo1.order_id)";
            ...
            if (is_array($t) && count($t) >= 1) {
                ...
                //  $custom_where  = ' OR (SELECT  mm.restaurant_name FROM mt_client mc
                //         LEFT JOIN mt_order mo ON mc.client_id = mo.client_id
                //         left join mt_merchant mm ON mo.merchant_id = mm.merchant_id
                //         WHERE mo.client_id = a.client_id
                //         AND mm.restaurant_name LIKE '."%farh%".'
                //         ORDER BY mo.order_id DESC LIMIT  1)';                               
            ...
            $sub_query2= "(
            SELECT  mm.restaurant_name FROM mt_client mc
            LEFT JOIN mt_order mo ON mc.client_id = mo.client_id
            left join mt_merchant mm ON mo.merchant_id = mm.merchant_id
            WHERE mo.client_id = a.client_id
            ORDER BY mo.order_id DESC LIMIT  1 
            ...
            $stmt = "
                SELECT SQL_CALC_FOUND_ROWS 
                a.*,  mm.restaurant_name,
                $sub_query                              
    
                FROM $sTable
                
                $sWhere
                AND mo1.order_id is null
          ...
          //  echo $stmt; exit('yes');
            ...
            if ($res = $this->rst($stmt)) {
                ...
                foreach ($res as $val) {
                    ...
                    $custom_query = "SELECT  mm.restaurant_name FROM mt_client mc
                    LEFT JOIN mt_order mo ON mc.client_id = mo.client_id
                    left join mt_merchant mm ON mo.merchant_id = mm.merchant_id
                    WHERE mo.client_id = " . $val['client_id'] . "
                    ORDER BY mo.order_id DESC LIMIT  1 ";
                    
                    $belong_to_merchant = '';
                    if($custom_query_res = $this->rst($custom_query)){
                            $belong_to_merchant = $custom_query_res[0]['restaurant_name'];    
                    }    
                    ...
                    $feed_data['aaData'][] = array(
                        ...
                        $val['social_strategy'],
                        $val['restaurant_name'],
                        '<div data-status="'.$val['email_notification'].'" onclick="checkEmailStatus('.$val['client_id'].','.$val['email_notification'].');" id="app_status_'.$val['client_id'].'" class="icheckbox_minimal '. $checked . ' style=" position:="" relative;"=""><input value="'.$val['email_notification'].'" class="icheck" checked="" type="checkbox" name="app_status" id="app_status" style="position: absolute; opacity: 0;"></div>',
        ...
        public function PopupMessages() {
            ...
            if (is_array($rows) && count($rows) >= 1) {
                foreach ($rows as $val) {
                    ...
                    if (!empty($val['status']) && $val['status']  == 'publish') {
                        $status = "<span class='tag'>Publish</span>";
                    } else
                        $status = "<span class='tag'>Draft</span>";

                    $feed_data['aaData'][] = array(
                        $chk, stripslashes($val['title']) . $option,
                        $status,
                        $date
                    );
                }
        ...
        /**
         * Popup Message List Admin
         */
        public function AdminPopupMessages() {
            $admin_id = Yii::app()->functions->getAdminId();
            $slug = $this->data['slug'];
            $stmt = "
            SELECT * FROM
         ...
         * Add Popup Message
         */
        public function addPopupMessage() {

            $latest_message = Yii::app()->functions->getLatestPopupMessage($_GET['id']);
            $sequence = $latest_message['sequence'];
            $params = array(
                'title' => addslashes($this->data['title']),
                'message' => $this->data['message'],
                'open_every_time' => isset($this->data['open_every_time']) ? addslashes($this->data['open_every_time']) : 0,
                'scheduled_message' => isset($this->data['scheduled_message']) ? addslashes($this->data['scheduled_message']) : 0,
                'schedule_time' => empty($this->data['schedule_time'])? '0000-00-00 00:00:00': date('Y-m-d H:i:s',strtotime($this->data['schedule_time'])),
                'status' => addslashes($this->data['status']),
                'date_created' => FunctionsV3::dateNow(),
                'ip_address' => $_SERVER['REMOTE_ADDR'],
                'merchant_id' => json_encode(Yii::app()->functions->getMerchantID()),
                'created_by' => Yii::app()->functions->getMerchantID()
            );
            $command = Yii::app()->db->createCommand();
            if (isset($this->data['id']) && is_numeric($this->data['id'])) {
                unset($params['date_created']);
                $params['date_modified'] = FunctionsV3::dateNow();
                $res = $command->update('{{popup_messages}}', $params,
                        'id=:id', array(':id' => addslashes($this->data['id'])));
                if ($res) {
                    $this->code = 1;
                    $this->msg = Yii::t("default", 'Message updated.');
                ...
                $params['sequence'] = $sequence+1;
                if ($res = $command->insert('{{popup_messages}}', $params)) {
                    $this->details = Yii::app()->db->getLastInsertID();
                    $this->code = 1;
                    $this->msg = Yii::t("default", 'Message added.');
                } else
                    $this->msg = Yii::t("default", 'ERROR. cannot insert data.');
            }
        }
         ...
         * Add Popup Message
         */
        public function addAdminPopupMessage() {

            $latest_message = Yii::app()->functions->getLatestPopupMessage($_GET['id']);
            $sequence = $latest_message['sequence'];
            $merchant_ids = $this->data['merchant_id'];
            // print_r($merchant_ids[0]);exit;
            if(empty($merchant_ids[0])){
                $merchant_ids = array();
                $merchant_list = FunctionsV3::merchantList();
            ...
            $params = array(
                ...
                'date_created' => FunctionsV3::dateNow(),
                'ip_address' => $_SERVER['REMOTE_ADDR'],
                'merchant_id' => json_encode($merchant_ids),
                'created_by' => Yii::app()->functions->getAdminId(),
                'user_type' => "admin"
            );
            $command = Yii::app()->db->createCommand();
            if (isset($this->data['id']) && is_numeric($this->data['id'])) {

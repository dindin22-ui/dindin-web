...
class Ajax extends AjaxAdmin 
{
	...
	public function merchantEnableReport(){
		
		$merchant_id = Yii::app()->functions->getMerchantID();	
		$res = $this->updateData("{{merchant}}", array('enable_report' => $_POST['enable_report'] ), 'merchant_id', $merchant_id);
		if ($res){	
				$this->code=1;
    			$this->msg=Yii::t("default","Setting saved");
		}
		else{
			$this->msg=Yii::t("default","Something went worng");
			
		}
			
	}

	public function merchantReportEmail(){
		
		$merchant_id = Yii::app()->functions->getMerchantID();
		$res = $this->updateData("{{merchant}}", array('report_email' => $_POST['report_email'] ), 'merchant_id', $merchant_id);
		if ( $res){	
				$this->code=1;
    			$this->msg=Yii::t("default","Setting saved");
		}
	...
	{		 		    
	    	...
	    	(
	    	select sum(cart_tip_value) 
	    	from
	    	{{order}}
	    	where merchant_id = a.merchant_id and doordash_drive_tracking_link != ''
	    	$and_date  		 
	    	) as tip_value,
	    	...
	    	{{order}}
	    	where merchant_id = a.merchant_id 	 
	    	$and_date AND trans_type = 'delivery'   		 
	    	) as total_delivery_orders_count
...
$action="<a href=\"$link\" >".Yii::t("default","Details")."</a>";
	    			...
	    			// $totalMinusCommission = $val['total_order']- $val['total_commission'];
	    			// $totalMinusCommission = $totalMinusCommission-$val['doordash_fee'];
	    			// $totalMinusCommission = $totalMinusCommission-$val['tip_value'];
                    $totalMinusCommission = $val['total_order']-$total_commission_row;
	    			$feed_data['aaData'][]=array(
	    			  $val['merchant_id'],
	    			  stripslashes($val['merchant_name']),
	    			  displayPrice(adminCurrencySymbol(),normalPrettyPrice($val['total_order'])),
	    			  displayPrice(adminCurrencySymbol(),normalPrettyPrice($total_commission_row)),
	    			  displayPrice(adminCurrencySymbol(),normalPrettyPrice($totalMinusCommission)),
	    			  $action	    			  
	    		    );
	    		}	    		
	    		
	    		$feed_data['total_commission']=displayPrice(adminCurrencySymbol(),normalPrettyPrice($total_commission_row));
	...
	**/
	...
	public function merchantCommission2()
	{		 		    
		    $where_params='';
		    $and_params='';
		...
		if ( $res=$this->rst($stmt)){		
			...
			foreach ($res as $val) {	
					...
					if($time < $store_close_date && $time > $store_start_date){
						...
						$resume_order_url=baseUrl()."/admin/resume-taking-order?merchant_id=".$val['merchant_id'];
						$resume_order_link='<br/><br/>
                					<a class="resume_order" rev="'.$val['merchant_id'].'" href="javascript:;"><div class="uk-badge">'.t("Resume Order").'</div></a>
                					';
					} 
					$status = $val['app_status'] =="1"?'checked':'';
					if (getOptionA('home_search_mode')!="postcode"){
						$feed_data['aaData'][]=array(
						  $val['merchant_id'],stripslashes($val['restaurant_name']).$action,
                                                  '<div data-status="'.$val['app_status'].'" onClick="checkMerchantAppStatus('.$val['merchant_id'].','.$val['app_status'].');" id="app_status_'.$val['merchant_id'].'" class="icheckbox_minimal '.$status.' style="position: relative;">'
                                                    . '<input value="'.$val['app_status'].'" class="icheck" '.$status.' type="checkbox" name="app_status" id="app_status" style="position: absolute; opacity: 0;">'
                                                    . '</div>',
						  $val['street'],
						  $val['city'],
						  $val['country_code'],
						  ...
						  FunctionsV3::merchantOpenTagAdmin($merchant_id)."".$resume_order_link,
	...
	public function InitPlaceOrder()
	{
	    ...
	    if ( $this->data['delivery_type']=="delivery"){
	    	if (FunctionsV3::isSearchByLocation()){	    		
                ...
                $merchant = FunctionsV3::getMerchantInfo($mtid);
...
//                if(isset($_SESSION['doordash_drive']) && $merchant['service'] == 8) {
...
//                    $result = FunctionsV3::estimateDoordashCharges($delivery_time, $sub_total, $mtid, $this->data);
//                    $this->msg = t("Sorry this merchant does not deliver to your location");
//                    rerutn;
//                    if($result['code'] == 2) {

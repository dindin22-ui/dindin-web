...
class FunctionsV3
{
	...
	public static function SumOfTotal()
	{
		$DbExt = new DbExt;
		$merchant_id = Yii::app()->functions->getMerchantID();

		$stmt = "SELECT SUM(total_w_tax) as total, SUM(packaging) as conveniencefee, SUM(total_commission) as commission , SUM(cart_tip_value) as tips
    	from
    	{{order}}   
    	where
		...
		$res = $DbExt->rst($stmt);
		if ($res) {
			$res[0]['total'] = prettyFormat($res[0]['total'], $merchant_id);
			$res[0]['total'] = ltrim($res[0]['total'], '0');
			$res[0]['conveniencefee'] = prettyFormat($res[0]['conveniencefee'], $merchant_id);
			$res[0]['commission'] = prettyFormat($res[0]['commission'], $merchant_id);
			$res[0]['tips'] = prettyFormat($res[0]['tips'], $merchant_id);
		}

		return $res[0];
	}

	public static function SumOfTotalSalesReport($merchant_id, $start_date, $end_date, $trans_type)
	{
		$DbExt = new DbExt;
		if ($trans_type == 'dindin') {
			$trans_type = 'Paid';
		}

		$stmt = "SELECT SUM(merchant_earnings) as merchant_total,SUM(total_w_tax) as total, SUM(packaging) as conveniencefee, SUM(total_commission) as commission , SUM(cart_tip_value) as tips
    	from
    	{{order}}   
    	where
    	merchant_id= " . FunctionsV3::q($merchant_id) . "
    	 and REPLACE(LOWER(status), ' ', '') = '" . $trans_type . "'
    	 and status NOT IN ('" . initialStatus() . "')
    	  and date_created BETWEEN '" . $start_date . "' and '" . $end_date . "'";

		$res = $DbExt->rst($stmt);
		if ($res[0]['total'] > 0) {
			$total = $res[0]['total'] - $res[0]['commission'];
			$res[0]['total'] = prettyFormat($total, $merchant_id);
			$res[0]['total'] = ltrim($res[0]['total'], '0');
			$res[0]['merchant_total'] = prettyFormat($res[0]['merchant_total'], '0');
			$res[0]['conveniencefee'] = prettyFormat($res[0]['conveniencefee'], $merchant_id);
			$res[0]['commission'] = prettyFormat($res[0]['commission'], $merchant_id);
			$res[0]['tips'] = prettyFormat($res[0]['tips'], $merchant_id);
		} else {
			$res[0]['total'] = strval(0);
		}
		if ($trans_type == 'dindin') {

			if (is_null($res[0]['merchant_total'])) {
				$res[0]['merchant_total'] = strval(0);
			}
			return $res[0]['merchant_total'];
		} else
			return $res[0]['total'];
	}

	public static function getUberEatsToken($scope = 'eats.store.orders.read')
	{
		$curl = curl_init();
		...
		curl_setopt_array($curl, array(
			...
			CURLOPT_POSTFIELDS => 'client_id=sU36y_ecO1luSfFKRiMVlJvhi-FQQQrn&client_secret=cWwDFTKv58OY1uIYSE4j9SG5xzA8UGS3QoYtC_7l&grant_type=client_credentials&scope=' . $scope,
	...
	public static function updateRestaurantStatus($store_id, $token, $status = 'PAUSED')
	{
		$curl = curl_init();

		curl_setopt_array($curl, array(
			CURLOPT_URL => 'https://api.uber.com/v1/eats/store/' . $store_id . '/status',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
	...
	public static function getUberEatsOrders($store_id, $tocken)
	{

		$curl = curl_init();

		curl_setopt_array($curl, array(
			CURLOPT_URL => 'https://api.uber.com/v1/eats/stores/' . $store_id . '/created-orders?limit=100',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
	...
	public static function getUberEatsOrderDetails($order_id, $token)
	{
		$curl = curl_init();

		curl_setopt_array($curl, array(
			CURLOPT_URL => 'https://api.uber.com/v2/eats/order/' . $order_id,
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
	...
	public static function m_log($arMsg)
	{
		...
		$fHandler = fopen('/home/orderdindin/public_html/' . $stCurLogFileName, 'a+');
		//write the info into the file  
		fwrite($fHandler, $stEntry);
		//close handler  
		fclose($fHandler);
	}
	public function acceptUberEatsOrder($order_id, $token)
	{
		$curl = curl_init();

		curl_setopt_array($curl, array(
			CURLOPT_URL => 'https://api.uber.com/v1/eats/orders/' . $order_id . '/accept_pos_order',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
	...
	public static function enableReports()
	{
		$DbExt = new DbExt;
		$stmt = "SELECT enable_report FROM {{merchant}} WHERE merchant_id = " . Yii::app()->functions->getMerchantID();
		$res = $DbExt->rst($stmt);
		return $res[0]['enable_report'];
	}
	public static function reportEmails()
	{
		$DbExt = new DbExt;
		$stmt = "SELECT report_email FROM {{merchant}} WHERE merchant_id = " . Yii::app()->functions->getMerchantID();
		$res = $DbExt->rst($stmt);
		return $res[0]['report_email'];
	}
	...
	public static function hccprinterReceipt($merchant_id, $order_id, $data)
	{
		...
		$server = "http://cloudprint.easyprt.com/o2o-print/print.php";
...
// 	    exit('ccccc'); 
	...
	public static function getMerchantLogo($merchant_id = '', $logo = '')
	{
		...
		if (!empty($merchant_logo)) {
			if (file_exists($upload_path . "/" . $merchant_logo)) {
				...
				$merchant_logo = assetsURL() . "/images/default-image-merchant.png";
		} else
			$merchant_logo = assetsURL() . "/images/default-image-merchant.png";
				...
				foreach ($filter_cuisines as $val) {
					if (!empty($val)) {
						...
						if ($x == 1) {
							...
							$filter_cuisine .= " OR cuisine LIKE " . FunctionsV3::q("%$val%") . " ";
	...
	public static function merchantOpenTagAdmin($merchant_id = '')
	{
		$is_merchant_open = Yii::app()->functions->isMerchantOpen($merchant_id);
		$merchant_preorder = Yii::app()->functions->getOption("merchant_preorder", $merchant_id);
		...
		if ($is_merchant_open) {
			$tag = '<span class="uk-button uk-button-success">' . t("Open") . '</span>';
		} else {
			if ($merchant_preorder) {
				$tag = '<span  id="merchant_' . $merchant_id . '" class="uk-button uk-button-primary">' . t("Pre-Order") . '</span>';
			} else {
				$tag = '<span  id="merchant_' . $merchant_id . '" class="uk-button uk-button-danger">' . t("Closed") . '</span>';
			}
		}
		$store_start_date = getOption($merchant_id, 'store_start_date');
		$store_close_date = getOption($merchant_id, 'store_close_date');
	...
	public static function isMerchantcanCheckout($merchant_id = '')
	{
		...
		$store_start_date = getOption($merchant_id, 'store_start_date');
		$store_close_date = getOption($merchant_id, 'store_close_date');
		$end_time_val = getOption($merchant_id, 'end_time_val');
		$time = time();
		if ($time < $store_close_date && $time > $store_start_date) {
			// $timet = $time.' '.date('Y-m-d H:i',$store_start_date).' '.date('Y-m-d H:i',$store_close_date);
			$msg = t("Sorry, we are currently not taking orders. Please check back later");
			$code = 2;
		}
		return array(
			'code' => $code,
		...
		if (is_array($prices) && count($prices) >= 1) {
			...
			if (!empty($size)) {
				...
				if ($discount >= 1) {
					...
					return qTranslate($size, 'size', $prices[0]) . " " . self::prettyPrice($regular_price);
			} else {
				if ($discount >= 1) {
					$original_price = unPrettyPrice($regular_price) + unPrettyPrice($discount);
	...
	public static function getFoodDefaultImage($photo = '', $small = true)
	{
		...
		if (!file_exists($path) || empty($photo)) {
			if ($small) {
				...
				$url_image = websiteUrl() . "/assets/images/default-food-image-large.png";
		} else
			$url_image = websiteUrl() . "/upload/$photo";
		return $url_image;
	}
	...
	public static function getMerchantOpeningHoursArray($merchant_id = '')
	{
		$stores_open_day = Yii::app()->functions->getOption("stores_open_day", $merchant_id);
		$stores_open_starts = Yii::app()->functions->getOption("stores_open_starts", $merchant_id);
		$stores_open_ends = Yii::app()->functions->getOption("stores_open_ends", $merchant_id);
	...
	public static function getAvatar($client_id = '')
	{
		...
		if (file_exists($path)) {
			...
			return assetsURL() . "/images/avatar.jpg";
	}
	...
	public static function getTopMenuActivated()
	{
		...
		if (empty($theme_top_menu)) {
			...
			$theme_top_menu = json_decode($theme_top_menu, true);
	...
	public static function sendMobilelVerificationCode($to = '', $code = '', $info = '')
	{
		// this is old start
		if (empty($to)) {
			return false;
	...
	public static function sendMobilelVerificationCodeNew($to = '', $code = '', $info = '')
	{
		// this is old start
		if (empty($to)) {
			return false;
		...
		$lang = Yii::app()->language;
		$enabled = getOptionA("customer_verification_code");
		if ($enabled == 1) {
			$subject = getOptionA("customer_verification_code_email_tpl_subject_$lang");
			if (!empty($subject)) {
				$subject = self::smarty('firstname', isset($info['first_name']) ? $info['first_name'] : '', $subject);
	...
	//this is old end
	public static function sendEmailOrderReport($to = '', $info = '')
	{
		if (empty($to)) {
			return false;
		}
		...
		$lang = Yii::app()->language;
		$subject = "Refund order request ";
		$tpl = "<h3>Hi Admin</h3>";
		$tpl .= "<p>\n You have refund order request #" . $info['order_id'] . "";
		$tpl .= "\n \n Customer name: " . $info['first_name'] . " " . $info['last_name'];
		$tpl .= "\n \n Customer email: " . $info['email_address'] . "";
		$tpl .= "\n \n Restaurant name: " . $info['merchant_name'] . "";
		if (!empty($info['contact_phone'])) {
			$tpl .= "\n \n Phone number: " . $info['contact_phone'] . "";
		}
		$tpl .= "\n \n Purchase Date: " . $info['date_created'] . "";
		$tpl .= "\n \n Refund Type: " . $info['type'] . "";
	...
	public static function timeListDelievery()
	{
		$options_al = array();
	...
	public static function timeList($merchant_id = 0)
	{
		$timezone = Yii::app()->functions->getOption("merchant_timezone", $merchant_id);
		$merchant_time_interval = Yii::app()->functions->getOption("merchant_time_interval", $merchant_id);
		date_default_timezone_set($timezone);

		$website_time_picker_format = getOptionA('website_time_picker_format');
		$asap = Yii::app()->functions->getOption('order_asap', $merchant_id);
		$options[""] = "";
		$min30 = array('00', '15', '30', '45');
		if ($asap == 3) {
			if ($merchant_time_interval == '20') {
				$min30 = array('00', '20', '40');
			} elseif ($merchant_time_interval == '25') {
				$min30 = array('00', '25', '50');
			}
		}
		$range = range(0, 23);
		if ($merchant_id !== 0) {
			$stores_open_ends = Yii::app()->functions->getOption("stores_open_ends", $merchant_id);
			$opening = $stores_open_ends = json_decode($stores_open_ends, true);
			$stores_open_ends = $stores_open_ends[strtolower(date('l'))];
			// print_r($stores_open_ends);
			$hours = explode(':', $stores_open_ends);
			$start_hour = date('H');
			$end_hour = $hours[0];
			$end_minute = date('i');
			$end_hour = $end_hour - 1;
			$range = range($start_hour, $end_hour);
			//                    echo $range;exit;
		}
		$i = 0;
		foreach ($range as $fullhour) {

			if ($website_time_picker_format == "12") {
				$parthour = $fullhour >= 12 ? $fullhour - 12 : $fullhour;
			} else
				$parthour = $fullhour >= 12 ? $fullhour - 0 : $fullhour;
			...
			foreach ($min30 as $int) {
				...
				if ($fullhour > 11) {
					...
					if ($website_time_picker_format == "12") {
						...
						$options[$parthour . ":" . $int . ""] = $parthour . ":" . $int . "";
				} else {
					if ($fullhour == 0) {
						$parthour = '12';
					}
		...
		foreach ($options as $option) {
			$hour = date('g');
			$minute = date('i') + $merchant_time_interval;
			$am_pm = date('A');
			if ($minute >= 60) {
				$minute = $minute - 60;
				if ($minute < 10)
			...
			$first_time = '';
			$first_time = $hour . ':' . $minute . ' ' . $am_pm;
			if ($i == 1) {
				$options_al[$first_time] = $first_time;
			} else if (strtotime($option) > strtotime($first_time)) {
				$options_al[$option] = $option;
			}
			$last_item = $option;
			// echo $last_item.' '.$option.' '.date('g:i A').'<br/>'; 
			$i++;
		}
		// print_r($opening);
		$opening = $opening[strtolower(date('l'))];
		// echo  date('g:i A',strtotime($opening)).' '.$opening;
		for ($i = 0; $i < 7; $i++) {
			$last_item = date('g:i A', strtotime('+15 minutes', strtotime($last_item)));
			if (date('Y-m-d G:i', strtotime($last_item)) <= date('Y-m-d G:i', strtotime($opening))) {
				$tokenize_date = explode(':', $last_item);
				if ($tokenize_date[0] == 12) {
					break;
				} else
					$options_al[$last_item] = $last_item;
			} else {
				break;
			}
		}
		// do {
		// 	echo $last_item = date('g:i A',strtotime('+15 minutes',strtotime($last_item)));
		// 	$options_al[$last_item] = $last_item;
		//   } while (strtotime($last_item) <= strtotime($opening));
		return $options_al;
	}
	public static function timeListCat($merchant_id = 0)
	{
		$timezone = Yii::app()->functions->getOption("merchant_timezone", $merchant_id);
		date_default_timezone_set($timezone);
		...
		foreach ($range as $fullhour) {
			if ($fullhour > 6) {
				...
				foreach ($min30 as $int) {
					if ($i == 0 && $int < date('i') && $merchant_id !== 0) {
						continue;
					}
					//                    $int = $int+15;
//                    if($int == 60){
...
//                    }
	...
	public static function latToAdress($lat = '', $lng = '')
	{
		...
		if ($protocol == "http") {
			...
			$api = "https://maps.googleapis.com/maps/api/geocode/json?latlng=" . urlencode($lat_lng);
	...
	public static function getMollieApiKey($admin = true, $merchant_id = '')
	{
		...
		if ($admin) {
			...
			if ($admin_mol_mode == "sandbox") {
				...
				$apikey = getOptionA('admin_mollie_apikey_live');
		} else {
			...
			if ($admin_mol_mode == "sandbox") {
				...
				$apikey = Yii::app()->functions->getOption('merchant_mollie_apikey_live', $merchant_id);
		}
		return $apikey;
	}
	...
	public static function dateNow2()
	{
		return date('Y-m-d');
	}
	...
	public static function prettyPaymentType($transaction_type = '', $payment_code = '', $data = '', $trn_type = '')
	{
		...
		if ($payment_code == "mol") {
			...
			return strtoupper(t($payment_code));
	}
	...
	public static function updateMerchantUsersUnsub($client_id = '')
	{
		// Secret key to decrypt
		$salt = "MY_SECRET_KEY_12345";
		$decrypted_id_raw = base64_decode($client_id);
		...
		$db_ext = new DbExt;
		$result = $db_ext->updateData("{{client}}", $params, 'client_id', $decrypted_id);
		return $result;
	}

	public static function getMerchantUsers($merchant_id = '')
	{
		$DbExt = new DbExt;
		// $merchant_details = "SELECT merchant_id, restaurant_name FROM {{merchant}} WHERE merchant_id in ($merchant_id)";
		// $res_details = $DbExt->rst($merchant_details);
		// $stmt = "SELECT *  
		// FROM {{order}} 
		// INNER JOIN {{client}} on {{order}}.client_id  = {{client}}.client_id 
		// where {{order}}.merchant_id in ($merchant_id) 
		// GROUP BY mt_client.email_address";

		$stmt = "SELECT SQL_CALC_FOUND_ROWS *,{{order}}.client_id,{{order}}.merchant_id FROM {{client}} 
            LEFT JOIN {{order}} ON {{client}}.client_id = {{order}}.client_id
            LEFT JOIN mt_merchant ON mt_order.merchant_id = mt_merchant.merchant_id
            LEFT JOIN {{order}} ord ON ({{order}}.client_id=ord.client_id AND {{order}}.order_id<ord.order_id) 
            WHERE {{order}}.merchant_id IN ($merchant_id) AND ord.order_id is null
	...
	public static function getSubscribedMerchantUsers($merchant_id = '')
	{
		$DbExt = new DbExt;
		// $merchant_details = "SELECT merchant_id, restaurant_name FROM {{merchant}} WHERE merchant_id in ($merchant_id)";
		// $res_details = $DbExt->rst($merchant_details);
		...
		$stmt = "SELECT SQL_CALC_FOUND_ROWS *,{{order}}.client_id,{{order}}.merchant_id FROM {{client}} 
            ...
            LEFT JOIN mt_merchant ON mt_order.merchant_id = mt_merchant.merchant_id
            LEFT JOIN {{order}} ord ON ({{order}}.client_id=ord.client_id AND {{order}}.order_id<ord.order_id) 
            WHERE {{client}}.unsubscribed = '0' AND {{order}}.merchant_id IN ($merchant_id) AND ord.order_id is null
            GROUP BY {{client}}.email_address";
	...
	public static function smarty($search = '', $value = '', $subject = '', $printer_flag = '')
	{
		...
		if ($printer_flag) {
			if ($search == 'sitename') {
				...
				$n = str_replace("[" . $search . "]", "\x1B\x61\x01\x1b\x4d\x01\x1d\x21\x11" . $value . "\n", $subject);
				// $n=str_replace("{".$search."}","\x1B\x61\x01\x1b\x4d\x01\x1d\x21\x11".$value."\n\n",$n);

				$n = str_replace("Transaction type: DELIVERY", "\x1B\x61\x01\x1b\x4d\x01\x1d\x21\x11Transaction type: DELIVERY\n", $n);

				$n = str_replace("Transaction type: PICKUP", "\x1B\x61\x01\x1b\x4d\x01\x1d\x21\x11Transaction type: PICKUP\n", $n);

			}

			if ($search == 'site_address') {
				//  echo $value; exit('hi');
	...
	public static function notifyStopTakingOrder($subject, $body)
	{
		//		$to = 'cs@omnitech.pro'; 
		$to = Yii::app()->functions->getOptionAdmin('notify_email');
		$DbExt = new DbExt();
		$lang = Yii::app()->language;
		$sender = getOptionA("global_admin_sender_email");
		$params = array(
	...
	public static function notifyStopTakingOrderSMS($subject, $body)
	{

		$DbExt = new DbExt();
		$number = Yii::app()->functions->getOptionAdmin('numbers_for_multiple_updates');
		...
		if ($number != '') {
			...
			foreach ($numbers as $key => $number) {
				...
				$d = $DbExt->insertData("{{sms_broadcast_details}}", $paramssms);
				//   print_r($d); exit('k');
			}
			if (!empty($subject) && !empty($body)) {
	...
	public static function notifyOmniTech($subject, $body)
	{
		$to = 'cs@omnitech.pro';
		$DbExt = new DbExt();
		$lang = Yii::app()->language;
	...
	public static function getLanguageFlag($lang = '')
	{
		...
		if (file_exists($flag)) {
			...
			$flag_link = assetsURL() . "/images/flags/us.png";
		return $flag_link;
	}
	...
	public static function getPushNotificationLink($merchant_id)
	{
		$DbExt = new DbExt;
		$stmt = "SELECT * FROM
	    	  ...
	    	  merchant_id=" . self::q($merchant_id) . "
	    	  AND
	    	  push_link != '' AND push_link_processed = 1
	    	  LIMIT 0,1
	    	  ";
		if ($res = $DbExt->rst($stmt)) {
			$push_logs = $res[0];
			$params_update = array(
				'push_link_processed' => 0
			);
			if (isset($_GET['debug'])) {
				dump($params_update);
			}
			$DbExt->updateData('{{mobile_merchant_pushlogs}}', $params_update, 'id', $push_logs['id']);
			return $push_logs;
		}
		return false;
	}
	...
	public static function merchantListBroadCost($as_list = true, $with_select = false)
	{
		$data = array();
		$DbExt = new DbExt;
		$stmt = "SELECT * FROM
    	...
    	ORDER BY restaurant_name ASC
    	";
		$data[''] = t("Please select a merchant");
		if ($res = $DbExt->rst($stmt)) {
			if ($as_list == TRUE) {
				foreach ($res as $val) {
					$data[$val['merchant_id']] = ucwords(stripslashes($val['restaurant_name']));
			...
			case 2:
				...
				if (is_array($payment_available) && count($payment_available) >= 1) {
					foreach ($payment_available as $payment_key) {
						...
						//   echo $master_key_val." ".$payment_key.'stp='.$payment_list[$payment_key]."\n";
						//   echo $master_key_val;
						if ($master_key_val == 1) {
	...
	public static function extraDeliveryFee($mtid = '', $current_fee = '', $delivery_time = '', $delivery_date = '')
	{
		...
		if (empty($delivery_time)) {
			...
			$delivery_time = date("Gi", strtotime($delivery_time));
	...
	public static function estimateDoordashCharges($delivery_time, $sub_total, $merchant_id, $data)
	{

		$curl = curl_init();
		$merchant = FunctionsV3::getMerchantInfo($merchant_id);
		$doordash_api_mode = Yii::app()->functions->getOptionAdmin("drive_api_mode");
		$subtotal = (int) $sub_total * 100;
		...
		curl_setopt_array($curl, array(
			CURLOPT_URL => 'https://api.doordash.com/drive/v1/estimates',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
			...
			CURLOPT_POSTFIELDS => '{
                                      "pickup_address": {
                                        ...
                                        "unit": "",
                                        "zip_code": "' . $merchant['post_code'] . '"
                                      },
                                      "dropoff_address": {
                                        "city": "' . $data['city'] . '",
                                        "state": "' . $data['state'] . '",
                                        "street": "' . $data['street'] . '",
                                        "unit": "' . $data['location_name'] . '",
                                        "zip_code": "' . $data['zipcode'] . '"
                                      },
                                      "order_value": ' . $subtotal . ',
                                      "delivery_time": "' . $delivery_time . '",
                                      "external_store_id": "' . $delivery_time . '",
                                      "external_business_name": "' . $merchant['restaurant_name'] . '"
                                    }',
			CURLOPT_HTTPHEADER => array(
		...
		//        print_r($data);
		$return['code'] = 2;
		$_SESSION['doordash_drive']['delivery_charges'] = 0;
		$_SESSION['doordash_drive']['delivery_time'] = '';
		$_SESSION['doordash_drive']['pickup_time'] = '';
		$validateDoordash = FunctionsV3::validateDoordashDelivery($delivery_time, $sub_total, $merchant_id, $data);
		if ($validateDoordash->valid == false) {
			$return['msg'] = $validateDoordash->errors[0][1];
		} else if (isset($result->field_errors)) {
			$return['msg'] = $result->field_errors[0]->error;
		} else {
			//            $shipping_fee = (float)$result->fee/100;
//            $_SESSION['doordash_drive']['delivery_charges'] = $shipping_fee;
			$_SESSION['doordash_drive']['delivery_time'] = $result->delivery_time;
			$_SESSION['doordash_drive']['pickup_time'] = $result->pickup_time;
			$return['code'] = 1;
		}
		return $return;
	}

	public static function validateDoordashDelivery($delivery_time, $sub_total, $mtid, $customer)
	{
		$doordash_api_mode = Yii::app()->functions->getOptionAdmin("drive_api_mode");
		$doordash_api_key = $doordash_api_mode == 'sandbox' ? Yii::app()->functions->getOptionAdmin("drive_api_key_sandbox") : Yii::app()->functions->getOptionAdmin("drive_api_key_live");
		$merchant_info = FunctionsV3::getMerchantInfo($mtid);
		$tip_val = 0;
		if ($_POST['tip_value'] > 0) {
			$tip_val = $_POST['tip_value'];
			$tip_val = (int) $tip_val * 100;
		} else {
			$tip_val = $_POST['cart_tip_percentage'] * $sub_total;
			$tip_val = (int) $tip_val * 100;
		}
		//        echo '<pre>';
//        print_r($customer);
...
//        ini_set("display_errors",true);
		$db = new DbExt();
		$subtotal = (int) $sub_total * 100;
		$item_count = count($_SESSION['kr_item']);
		$curl = curl_init();

		//        $customer_phone = explode('+1',$customer['contact_phone']);
//        $customer_phone = $customer_phone[1];
		...
		$result = $db->updateData('{{client}}', $params, 'email_address', $customer['email_address']);
		curl_setopt_array($curl, array(
			CURLOPT_URL => 'https://api.doordash.com/drive/v1/validations',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
			...
			CURLOPT_POSTFIELDS => '{
                      "pickup_address": {
                        ...
                        "unit": "",
                        "zip_code": "' . $merchant_info['post_code'] . '"
                      },
                      "pickup_phone_number": "' . $merchant_info['contact_phone'] . '",
                      "pickup_instructions": "",
                      "dropoff_address": {
                        "city": "' . $customer['city'] . '",
                        "state": "' . $customer['state'] . '",
                        "street": "' . $customer['street'] . '",
                        "unit": "' . $customer['location_name'] . '",
                      ...
                      "dropoff_instructions": "' . $_POST['delivery_instruction'] . '",
                      "order_value": "' . $subtotal . '",
                      "tip": ' . $tip_val . ',
                      "delivery_time": "' . $delivery_time . '",
                      ...
                      "external_store_id": "' . $merchant_info['merchant_id'] . '",
                      "external_business_name": "' . $merchant_info['restaurant_name'] . '",
	...
	public static function createDoordashDelivery($delivery_date, $data, $merchant_info, $time_in_selected, $confirmed)
	{

		//        echo '<pre>';
		//        print_r($data);
		$customer = FunctionsV3::getCustomerDetails($data['client_id']);//$_SESSION['kr_client'];
		//        print_r($_SESSION);
		//        print_r($customer);
		//        echo '</pre>';
		//        return;
		...
		$doordash_api_mode = Yii::app()->functions->getOptionAdmin("drive_api_mode");
		$doordash_api_key = $doordash_api_mode == 'sandbox' ? Yii::app()->functions->getOptionAdmin("drive_api_key_sandbox") : Yii::app()->functions->getOptionAdmin("drive_api_key_live");
		$subtotal = (int) ($data['sub_total'] * 100);
		if ($data['payment_type'] == 'cod') {
			$cash_on_delivery = $subtotal;
		}
		$curl = curl_init();
		$delivery_start_date = $_SESSION['kr_delivery_options']['delivery_start_date'] . "T" . $_SESSION['kr_delivery_options']['delivery_start_time'] . ':00Z';
		//        $delivery_date = $_SESSION['kr_delivery_options']['delivery_date']."T".$_SESSION['kr_delivery_options']['delivery_time'].':00Z';

		$cart_item = !empty($data['json_details']) ? json_decode($data['json_details'], true) : false;
		...
		if (isset($data['tip_enabled'])) {
			if ($data['tip_enabled'] == 2 && $data['tip_percent'] >= 0.0001) {
				$data['cart_tip_value'] = $data['sub_total'] * ($data['tip_percent'] / 100);
			}

			$tip = $data['cart_tip_value'] * 100;
		}
		Yii::app()->functions->data = "list";
		$mid = $data['merchant_id'];
		$timezone = Yii::app()->functions->getOption("merchant_timezone", $mid);
			...
			if (is_array($cart_item) && count($cart_item) >= 1) {
				...
				foreach ($cart_item as $key => $val) {
					...
					if (Yii::app()->functions->getOptionAdmin("enabled_multiple_translation") == 2) {
						$food_info = Yii::app()->functions->getFoodItem($val['item_id']);
						...
						if (!empty($val['cooking_ref'])) {
							$cooking_ref_trans = Yii::app()->functions->getCookingTranslation($val['cooking_ref'], $mid);
						}
					} else
						$food_info = Yii::app()->functions->getFoodItem($val['item_id']);
					$food_item_name = isset($food_item[$val['item_id']]) ? $food_item[$val['item_id']] : '';
					$item_name = qTranslate(
						isset($food_item[$val['item_id']]) ? $food_item[$val['item_id']] : ''
						,
						'item_name',
						$food_infos
					);
					$total_price = (int) (($quantiy * (float) $price) * 100);
					$instructions = explode('<br/>', $val['instructions']);
					$cart_items[] = array(
						'name' => $food_item_name,
						'description' => $instructions,
						'barcode' => '',
						'quantity' => (int) $quantiy,
						'external_id' => $val['item_id'],
						'volume' => '',
						'weight' => '',
						'price' => $total_price
					);
		...
		//        $customer_phone = explode('+1',$data['contact_phone']);
		//        $customer_phone = $customer_phone[1];
		$customer_phone = $data['contact_phone'];
		$citems = json_encode($cart_items);
		curl_setopt_array($curl, array(
			CURLOPT_URL => 'https://api.doordash.com/drive/v1/deliveries',
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => '',
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
			...
			CURLOPT_POSTFIELDS => '{
											  ...
											  "dropoff_address": {
												...
												"unit": "' . $data['location_name1'] . '",
												"zip_code": "' . $data['client_zipcode'] . '"
											  },
											  "customer": {
												"phone_number": "' . $customer_phone . '",
												"business_name": "' . $customer['first_name'] . ' ' . $customer['last_name'] . '",
											  ...
											  "barcode_scanning_required": false,
											  "pickup_business_name": "' . $merchant_info['restaurant_name'] . '",
											  "pickup_instructions": "' . $data['delivery_service_special_instructions'] . '",
											  "dropoff_instructions": "' . $data['delivery_instruction'] . '",
											  "order_volume": 0,
											  "tip": ' . (int) $tip . ',
											  "external_delivery_id": "' . $data['order_id_token'] . '",
											  "driver_reference_tag": "1",
											  "external_business_name": "' . $merchant_info['restaurant_name'] . '",
											  "external_store_id": "' . $data['merchant_id'] . '",
											  "contains_alcohol": false,
											  "requires_catering_setup": false,
											  "num_items": ' . count($cart_item) . ',
		...
		if (isset($response->field_errors)) {
			$return['msg'] = $response->field_errors[0]->error;
			$return['code'] = 2;
		} else {
			//            echo 'success';
			//            print_r($response);
			if (isset($response->estimated_pickup_time)) {
				$estimated_pickup_time = FunctionsV3::convertUTCtoSQL($response->estimated_pickup_time, $data['merchant_id']);
				$estimated_delivery_time = FunctionsV3::convertUTCtoSQL($response->estimated_delivery_time, $data['merchant_id']);
				$doordash_drive_id = $response->id;
				$delivery_tracking_url = $response->delivery_tracking_url;
				date_default_timezone_set($timezone);
				$params = array(
					'delivery_date' => $estimated_delivery_time['date'],
					//on auto on we have added default time so it use defined times instead of doordash api estimated time.
					// 'delivery_time'=>$estimated_delivery_time['time'],
					//'delivery_time'=> date("h:i A", strtotime("+".$time_in_selected." minutes",  strtotime( date("Y-m-d h:i A") )  )),
					'confirmed' => $confirmed,
					'pickup_in' => $time_in_selected,
					'doordash_drive_pickup_date' => $estimated_pickup_time['date'],
					'doordash_drive_pickup_time' => $estimated_pickup_time['time'],
					//'doordash_drive_pickup_time'=> date("h:i A", strtotime("+".$time_in_selected." minutes",  strtotime( date("Y-m-d h:i A") )  )),
					'doordash_drive_tracking_link' => $delivery_tracking_url,
					'doordash_drive_id' => $doordash_drive_id
				);
				//                print_r($params);
				$result = $db->updateData('{{order}}', $params, 'order_id', $data['order_id']);
				$return['code'] = 1;
				//                print_r($result);
			}
		}
	...
	public static function createDoordashDelivery2($delivery_date, $data, $merchant_info, $time_in_selected, $confirmed)
	{
		...
		if (isset($response->field_errors)) {
			...
			if (isset($response->estimated_pickup_time)) {
				$estimated_pickup_time = FunctionsV3::convertUTCtoSQL($response->estimated_pickup_time, $data['merchant_id']);
				echo $estimated_delivery_time = FunctionsV3::convertUTCtoSQL($response->estimated_delivery_time, $data['merchant_id']);
				$doordash_drive_id = $response->id;
				$delivery_tracking_url = $response->delivery_tracking_url;
				date_default_timezone_set($timezone);

				//check auto confirm off(time slot will be coming)
	...
	public static function convertUTCtoSQL($unix_time, $merchant_id)
	{
		$date_time = explode('T', $unix_time);
		$date = $date_time[0];
		...
		$the_date = strtotime($unix_time);
		$date = date('Y-m-d', $the_date);
		$time = date('G:i', $the_date);
		//	    print_r(array('date'=>$date,'time'=>$time));
	...
	public static function getTopItems($merchant_id)
	{
		$DbExt = new DbExt;
	...
	public static function getCustomerDetails($client_id)
	{
		$db = new DbExt();
		$stmt = "
    	SELECT * FROM
			...
			if (array_key_exists($delivery_day, (array) $res)) {
				...
				if (!empty($time_based)) {
					...
					if (preg_match("/x/i", $time_based)) {
						...
						$time_based_ex = explode("-", $time_based);
	...
	public static function updateOrderPayment(
		...
		if (empty($status)) {
			$status = Yii::app()->functions->getOption("default_order_status", $merchant_id);
		}
		$db = new DbExt();
		$params_logs = array(
			'order_id' => $order_id,
	...
	public static function getCustomerList($id = '')
	{
		...
		$DbExt = new DbExt;
		$stmt = "SELECT COUNT('*') FROM {{order}} WHERE merchant_id IN ($id) AND client_id != 'null'  GROUP BY client_id;";
		if ($res = $DbExt->rst($stmt)) {
			return count($res);
		}
		return false;
	}
	public static function PopUpMessage($merchant_id)
	{
		$date = FunctionsV3::dateNow2();
		$DbExt = new DbExt;
		$stmt = "SELECT * FROM
			{{popup_messages}}
			WHERE
			...
			AND status = 'publish'
			AND (open_every_time = 1 OR schedule_time LIKE '" . $date . "%')
			ORDER BY id DESC			
		";
		if ($res = $DbExt->rst($stmt)) {
			if (count($res) > 0)
	...
	public static function getCustomerEmailList($id = '')
	{
		//echo $id;die;
		$DbExt = new DbExt;
		$stmt = "SELECT a.merchant_user_id,merchant_id,contact_email,contact_number,
        concat(first_name,' ',last_name) as full_name
       FROM
       {{merchant_user}} a
        WHERE
       merchant_user_id IN($id)";
		if ($res = $DbExt->rst($stmt)) {
			return $res;
		}
		return false;

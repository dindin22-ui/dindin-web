...
<?php
	...
	public static function timeList($merchant_id = 0)
	{
		...
		$options[""]=""; $min30=array('00','15','30','45');
                $range = range(0,23);
                if($merchant_id !== 0){
                    $stores_open_ends=Yii::app()->functions->getOption("stores_open_ends",$merchant_id);
...
//                    }
    ...
    public static function getSubscribedMerchantUsers($merchant_id='')
    {
        ...
        $stmt = "SELECT SQL_CALC_FOUND_ROWS *,{{order}}.client_id,{{order}}.merchant_id FROM {{client}} 
            ...
            LEFT JOIN mt_merchant ON mt_order.merchant_id = mt_merchant.merchant_id
            LEFT JOIN {{order}} ord ON ({{order}}.client_id=ord.client_id AND {{order}}.order_id<ord.order_id) 
            WHERE {{client}}.email_notification = '1' AND {{order}}.merchant_id IN ($merchant_id) AND ord.order_id is null
            GROUP BY {{client}}.email_address";
...
//		$to = 'cs@omnitech.pro'; 
		...
		$DbExt=new DbExt(); $lang=Yii::app()->language;   
		$sender=getOptionA("global_admin_sender_email");	
		$params=array(
			'email_address'=>$to,
			'sender'=>$sender,
        ...
        if (isset($cart_item)) {
            if (is_array($cart_item) && count($cart_item) >= 1) {
                ...
                foreach ($cart_item as $key => $val) {
                    ...
                    } else $food_info=Yii::app()->functions->getFoodItem($val['item_id']);
...
// print_r($data); exit('sac');
        // $customer_phone = '5045045280';
         $customer_phone = $data['contact_phone'];
      
         
         //here is the preoblem customer phone is not comming
...
//                print_r($result);
    ...
    //DoorDash  function for App Side
    	public static  function  createDoordashDeliveryAppSide($delivery_date, $data,$merchant_info,$time_in_selected,$confirmed){
//       echo '<pre>';
        // print_r($confirmed); exit('sd');
        $customer = FunctionsV3::getCustomerDetails($data['client_id']);//$_SESSION['kr_client'];
        
        
        
        // print_r($customer); exit('sds');
        // print_r($data); exit('sds');
...
//                print_r($result);
    ...
    //end App side function
    
    
    
    ///Test Functiom doordashhh api by AR
    
//     	public static  function  createDoordashDelivery($delivery_date, $data,$merchant_info,$time_in_selected,$confirmed,$client_id){
    	    
    	    
//     	   // print_r($client_id); exit('asdsa');
    	    
//     	   //$delivery_date = '2022-11-26T13:42:00Z';
...
//	    print_r(array('date'=>$date,'time'=>$time));
    ...
    $order_id,$payment_code='',$payment_ref='',$payment_response='', $merchant_ref='', $status = '')
    {
        $merchant_id=Yii::app()->functions->getMerchantID();

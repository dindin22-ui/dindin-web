...
{	
	...
	public function Services()
	{
		return array(
		  ...
		  6=>t("Pickup & Dinein"),
		  7=>t("Dinein Only"),
            8=>t("Doordash delivery and Pick up")
        );
	}
    ...
    public function jsLanguageAdmin()
    {
    	...
    	return array(
    	  ...
    	  "resumeWarning"=>Yii::t("default","You are about to resume orders the selected merchant.\n'Cancel' to stop, 'OK' to resume order.?"),
    	  ...
    	  'trans_53'=>t("SMS code is required"),
            "trans_54"=>t("Delivery Street is required"),
            "trans_55"=>t("Delivery City is required"),
            "trans_56"=>t("Delivery State is required"),
            "trans_57"=>t("Delivery Zipcode is required"),
            "trans_58"=>t("First Name is required"),
            "trans_59"=>t("Last Name Phone is required"),
            "trans_60"=>t("Email is required"),
	...
	/**
	 * Get Admin Popup Message by ID
	 */
	public function getPopUpMessageAdmin($id='')
    {
    	$mid=$this->getMerchantID();
    	$DbExt=new DbExt;
	    $stmt="SELECT * FROM
	...
	/**
	 * Get Latest PopupMessage
	 */
	public function getLatestPopupMessage()
    {
    	$mid=$this->getMerchantID();
    	$DbExt=new DbExt;
	    $stmt="SELECT * FROM
    ...
    public function getAddOnLists($merchant_id='')
	{
        ...
        public function getAddOnPrintLists($merchant_id='')
	{
		$data_feed=array();
		$stmt="
		SELECT * FROM
		...
		if (is_array($rows) && count($rows)>=1){
			if ($this->data=="list"){
				foreach ($rows as $val) {									   
				   //$data_feed[$val['cat_id']]=$val['category_name'];
                                   $data_feed[$val['subcat_id']]=$val['subcategory_print_name'];
				}
				return $data_feed;
			} else return $rows;
		}
	...
	public function merchantMenu()
	{
		...
		return array(  
		    ...
		    'items'=>array(
                ...
                array('visible'=>FunctionsV3::hasMerchantAccessToMenu($user_access,"Popup Messages"),'tag'=>"Merchant",'label'=>'<i class="fa fa-envelope-o"></i>'.Yii::t("default","Popup Messages"),
                'url'=>array('/merchant/PopupMessages')),
	...
	public function getItemDetailsByItemId($item_id='',$addon=false,$merchant_id='')
	{
		$DbExt=new DbExt;
		// $data=array();
			...
			foreach ($res as $cat_i=>$cat_name) {	
				$data[]=array(
				  ...
				  'category_description'=>$cat_name['category_description'],
                                'start_time'=>$cat_name['start_time'],
                                'end_time'=>$cat_name['end_time'],
	...
	public function displayOrderHTML($data='',$cart_item='',$receipt=false,$new_order_id='',$controller='')
	{
		$item_array=array();
		$this->code=2;
		$htm='';	
...
//        print_r($data);
        ...
        if($controller == 'api'){
            $subcat_list=Yii::app()->functions->getAddOnPrintLists($mid);	   
        }else{
...
//    				dump($val);				
    				...
    				$qty=$val['qty'];  /** fixed addon qty */   
                                  ...
                                  if($data['delivery_service_type'] == 'grubhub' || $data['delivery_service_type'] == 'doordash' || $data['delivery_service_type'] == 'ubereats'){
                                      $quantiy = $val['quantity'];
                                      $food_item = $item_name = $val['item_name'];
                                      $total_price = (float)$price;
                                  }else{
                                      $quantiy = $val['qty'];
                                     $total_price=$quantiy*(float)$price;
                                  }
    				...
    				if($data['delivery_service_type'] == 'grubhub' || $data['delivery_service_type'] == 'doordash' || $data['delivery_service_type'] == 'ubereats'){
                                      $quantiy = $val['quantity'];
                                      $food_item_name = $item_name = $val['item_name'];
                                      $total_price = (float)$price;
                                  }else{
                                      $quantiy = $val['qty'];
    				...
    				$htm.='<div class="item-order-list item-row" id="test-print">';
    				  ...
    				  if ($res_category=$this->getCategory($val['category_id'])){     
    				  	  ...
    				  	  'category_name',$category_name_trans)."</b></p>";
                                          $val['cat_print_name'] = qTranslate($res_category['category_name'],
    				  	  'category_name',$category_name_trans);
    				  } 
    				  
    				  //dump($htm);
			          $htm.='<div class="a">'.$quantiy.'</div>';
			          $htm.='<div class="b">'.$item_name;
			          if (!empty($size_words)){
			          ...
			          $item_array[$key]=array(
			            ...
			            'qty'=>$quantiy,
                                    'grubhub_instructions' => $instructions,  
			            ...
			            'category_name_trans'=>isset($category_name_trans)?$category_name_trans:'',	
                                    'print_cat_name'=>qTranslate($res_category['category_name'],'category_name',$category_name_trans), 
...
//                                            ."</p>";			          	  
			          ...
			          /*ingredients*/  
                                  if($data['delivery_service_type'] == 'grubhub' || $data['delivery_service_type'] == 'doordash' || $data['delivery_service_type'] == 'ubereats'){
                                      if(isset($val['instructions'])){
                                          $htm.="<p class=\"small\">".$val['instructions']."</p>";
                                      }
                                  }else{
			          	 		...
			          	 		if ($this->getOptionAdmin("enabled_multiple_translation")==2){
			          	 			if($subcategory_tran=$this->getAddonCategory($cat_id)){
			          	 				$subcategory_trans['subcategory_name_trans']=!empty($subcategory_tran['subcategory_name_trans'])?json_decode($subcategory_tran['subcategory_name_trans'],true):'';
                                                        ...
                                                        if(!empty(qTranslate($subcat_list[$cat_id],'subcategory_name',
						        $subcategory_trans))){
                                                            $htm.='<div class="a"></div>';
...
//                                                }
    			...
    			dump($subtotal);*/
                $merchant_info = FunctionsV3::getMerchantInfo($mid);
    			if(isset($_SESSION['doordash_drive'])){

//                    $delivery_charges = $_SESSION['shipping_fee'] = $merchant_info['doordash_fee'];// $_SESSION['doordash_drive']['delivery_charges'];
                }
    			//shipping rates
		        ...
		        $htm.='<div class="summary-wrap">';
		         ...
		         if ( $has_voucher==TRUE){		 
			         ...
			         displayPrice(baseCurrency(),prettyFormat($subtotal,$mid))
                                ...
                                $data['print_sub_total'] = prettyFormat($subtotal,$mid);
		         } else {			         
		         ...
		         if (!empty($merchant_packaging_charge)){		
		           if($merchant_packaging_charge>=0.001){         
			           $htm.=FunctionsV3::receiptRowTotal("Packaging",
				         ...
				         displayPrice(baseCurrency(),prettyFormat($tax_amt,$mid))
				         );
                                $total += $tax_amt;
                            //  }
                         }else{
		         ...
		         /*TIPS*/
                         if($data['delivery_service_type'] == 'grubhub' || $data['delivery_service_type'] == 'doordash' || $data['delivery_service_type'] == 'ubereats'){
                             $cart_tip_value_i = (int)$data['cart_tip_value'];
                             if($cart_tip_value_i > 0){
                                $htm.=FunctionsV3::receiptRowTotal( t("Tip") ,
	    	   ...
	    	   $htm.="<div class=\"col-md-6 col-xs-6  text-right\">".t("Total")."</div>";
                if($data['delivery_service_type'] == 'ubereats'){
                    $htm.="<div class=\"col-md-6 col-xs-6  text-right cart_total\">".
                        displayPrice(baseCurrency(),prettyFormat($data['total_w_tax'],$mid))."</div>";
                }else{
                    $htm.="<div class=\"col-md-6 col-xs-6  text-right cart_total\">".
		       ...
		       $item_array_total=array(
		         'subtotal'=>$subtotal,
                         'print_sub_total'=>$data['print_sub_total'],  
                            ...
                            $item_html = '';
							   ...
							   foreach($order_items as $order_item){
                                $item_html .= '<div class="item-order-list item-row" id="test-print">'
                                        ...
                                        . '<p class="uk-text-small"><span class="base-price">'.$order_item['instructions'].'</span></p></div><div class="manage">'
                                        . '<div class="c"></div>'
...
//                        echo $htm;
	...
	public function getCreditCardInfoByclient($client_id)
	{
		$stmt="
		SELECT * FROM
		{{client_cc}}
...
// 		dump($stmt);
    ...
    public function generateRandomKeyAlfa($range=6) 
    {
...
$menu_html.="</li>";
    ...
    public function getMerchantInfoByName($name){
        if(empty($name)){
            return false;
        }else{
            $db_ext=new DbExt;
                ...
                {{merchant}}
                WHERE
                restaurant_name Like ".Yii::app()->db->quoteValue('%'.$name.'%')."
                LIMIT 0,1
                ";
...
//                 print_r($res);
	...
	public function getMerchantHolidayTimeFrom($merchant_id='')
	{
		...
		if (!empty($stores_time_from)){
			$stores_time_from=json_decode($stores_time_from,true);
			if (is_array($stores_time_from) && count($stores_time_from)>=1){
			   return $stores_time_from;	
			}			
		}		
		return false;
	}
	...
	public function getMerchantHolidayTimeTo($merchant_id='')
	{
		$stores_time_to=Yii::app()->functions->getOption("stores_time_to",$merchant_id);
		if (!empty($stores_time_to)){
			$stores_time_to=json_decode($stores_time_to,true);
			if (is_array($stores_time_to) && count($stores_time_to)>=1){
			   return $stores_time_to;	
			}			
		}		
		return false;
	}
   ...
   public function getSubsriberEmail($email_address='')
	...
	{				
		...
		";		
		if ( $res=$this->db_ext->rst($stmt)){
		    $commision =  $res[0]['percent_commision'];
	...
	{				
		...
		";		
		if ( $res=$this->db_ext->rst($stmt)){
// 			echo "<pre>"; print_r($res[0]['any_fee']); exit('nnnnn'); 
			return $res[0]['any_fee'];
		}
		return false;
	}

    public function getMerchantDoordashFee($merchant_id='')
    {
        $stmt="
		SELECT * FROM
		{{merchant}}
			...
			if ($this->data=="list"){
				foreach ($rows as $val) {
                                    ...
                                    if($enabled_category_sked == 1){
                                        ...
                                        if(isset($val[$todays_day.'_end_time']))
                                            $end_date = $this->convertHours($val[$todays_day.'_end_time']);

...
//		         }
		   ...
		   <?php
                       if($data['delivery_service_type'] == 'grubhub' || $data['delivery_service_type'] == 'doordash' || $data['delivery_service_type'] == 'ubereats'){
                        ...
                        $item_html .= '<div class="summary-wrap">
                            <div class="row">
                            <div class="col-md-6 col-xs-6 text-right ">Sub Total</div><div class="col-md-6 col-xs-6 text-right cart_subtotal">$'.$data['sub_total'].'</div>
                                </div><div class="row"><div class="col-md-6 col-xs-6 text-right ">Tax</div><div class="col-md-6 col-xs-6 text-right ">$'.$data['tax'].'</div></div>
                                        <div class="row cart_total_wrap bold"><div class="col-md-6 col-xs-6  text-right">Total</div><div class="col-md-6 col-xs-6  text-right cart_total">$'.$data['total_w_tax'].'</div></div>
                                            <input type="hidden" value="'.$data['sub_total'].'" name="subtotal_order" id="subtotal_order">'
                                                . '<input type="hidden" value="'.$data['sub_total'].'" name="subtotal_order2" id="subtotal_order2">	   
		  </div>';
                    }else{
                        $phone = $data['contact_phone1'];
                        $item_html = $data2['html'];
                    }
                       ?>
		   <?php echo $item_html;?>

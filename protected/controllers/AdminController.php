...
class AdminController extends CController
{
	...
	*/
	...
	{
	   // $set = FunctionsV3::notifyStopTakingOrderSMS('test', 'hello zeeshan');
    ...
    public function actionResumeTakingOrder(){
		$mtid = $_REQUEST['merchant_id'];
		$store_start_date = '';	    
		...
		if ( $DbExt->updateData('{{mobile_device_merchant}}',$params,'merchant_id',$mtid)){
			...
			$message_out = 'You resumed your orders';						
		}else{
			$message_out = 'Something went wrong. Please try again';
		}
		$this->redirect(array('admin/merchant?message='.urlencode($message_out)));
	}
	...
	public function actionAdminPopupMessages()
	{	    
		$this->crumbsTitle=Yii::t("default","Popup Messages");
		
	    if (isset($_GET['Do'])){
			if ( $_GET['Do']=="Add"){
				$this->render('popup_messages_add');
			} elseif ( $_GET['Do'] == "Sort" ){	
			   $this->render('popup_sort');
			} else $this->render('popup_list');
		} else $this->render('popup_list');
	}	

...
class CronController extends CController
{
	...
	public function actionSendOrderAlert(){
		$DbExt = new DbExt;
		$stmt = "SELECT a.*
		...
		if ($res = $DbExt->rst($stmt)) {
			foreach ($res as $val) {
				$from_time = strtotime($val['date_created']);
				$to_time = time();
				echo $minutes =  round(abs($to_time - $from_time) / 60,2);
				if($minutes > 4.50){
					echo ceil($minutes);
					if (($minutes % 3 == 0) && $merchant_info=Yii::app()->functions->getMerchant($val['merchant_id'])) {
						$emails = getOptionA('non_confirmed_order_emails');
						$emails = explode(",",$emails);
						foreach($emails as $email){;
							$message = "Please call <b>".$merchant_info['restaurant_name']."</b> at ".$merchant_info['restaurant_phone']." to verify that they are open. They are not opening order. <br/><br/>Thank You";
							sendEmail($email,'',$merchant_info['restaurant_name']. " not opening orders",$message);
						}
					}
				}
			}
	...
	public function actionPauseTodayOrders(){
		$db_ext=new DbExt;
		$stmt="SELECT * FROM
		{{merchant}}
		";
		if ( $merchants=$db_ext->rst($stmt)){
			foreach($merchants as $merchant){
				...
				$m_holiday=Yii::app()->functions->getMerchantHoliday($merchant_id);
				...
				$i = 0;
				foreach($m_holiday as $h){
					if($h == date('Y-m-d')){
						$time_start = $m_holiday_from[$i];
						$time_close = $m_holiday_to[$i];
						$store_close_date = $h." ".$time_close;
						$store_close_date = date('Y-m-d G:i:s',strtotime($store_close_date));

						
						$store_start_date = $h." ".$time_start;
						$store_start_date = date('Y-m-d G:i:s',strtotime($store_start_date));
						$params=array(
						...
						$DbExt=new DbExt;
						if ( $DbExt->updateData('{{mobile_device_merchant}}',$params,'merchant_id',$merchant_id)){
							  
							if (isset($store_start_date)){
								Yii::app()->functions->updateOption("store_start_date",strtotime($store_start_date),$merchant_id);  
							}
							Yii::app()->functions->updateOption("end_time",strtotime($store_close_date),$merchant_id); 
							Yii::app()->functions->updateOption("store_close_message","Store is not taking order at the moment",$merchant_id);
...
//		FunctionsV3::m_log('Uber Eats Order Ended: '.date('Y-m-d H:i'));
	...
	public function actionFoodOrder(){ 
	   require ('/var/www/vhosts/orderdindin.com/httpdocs/vendor/autoload.php');
           require (Yii::app()->basePath.'/components/simple_html_dom.php');
            $client = new Google_Client();
            // $fromEmail = 'foodorders@omnitech.pro';
            $fromEmail = 'food@dindint.com';
            ...
            $client->setDeveloperKey("AIzaSyB81sDYgmtw337SBwnf0s3L8G0oTgA1ocU");
            $client->setAuthConfig(Yii::app()->basePath.'/components/food-order-364106-79b6e040491e.json');
            $client->setScopes(["https://mail.google.com/",
                "https://www.googleapis.com/auth/gmail.compose",
                "https://www.googleapis.com/auth/gmail.modify",
                "https://www.googleapis.com/auth/gmail.send",
...
// 			$opt_param['maxResults'] = 4;  //this limits the amount of unread messages returned
// 			$opt_param['labelIds'] =  'IMPORTANT';
	...
	// echo $text;exit;
								...
								// 	print_r($order_details);continue;
									$trans_type = 'Delivery';
											...
											if ( strpos(trim($order_details[$i]), 'Qty.') !== false)  {
												...
												for( ;$i <  count($order_details);$i++){
													...
													if ( strpos(trim($order_details[$i]), '~ End of Order ~') !== false)  {
													   // echo $order_details[count($order_details)-3];
														$i = $i+1;
													...
													if(empty($subtotal)){
														$subtotal_price = explode('$',$order_details[count($order_details)-3]);
														$subtotal = $subtotal_price[1];
													}
...
// echo $str;
						...
						if(count($confirm_link_array)>0)
						    $confirm_link = $confirm_link_array[0];
						if($status == 'grubhub'){
...
//							$data = htmlspecialchars_decode(strstr($data,'</html>',true));
				...
				// 			print_r($data);
				            ...
				            $items_div = $DOM->getElementsByTagName('div');
				             $return = array();
				             $table_count = count($items);
				             echo $table_count."Muzammil"; 
		                    $scheduled_order = false;
				             if($table_count < '9'){
				                 $i = 0;
                				foreach ($items_div as $node) {
                                    $tr = $node->childNodes;
                                    $str = array();
                                    foreach ($tr as $trelement) {
                                        if (count(explode('  ',trim($trelement->nodeValue))) > 1) {
                                            ...
                                            if($table_count < '7'){
                                                ...
                                                // }
                                                // if ($i == 55 || $i == 45) {
                                                //     $customer_details = explode('  ',$trelement->nodeValue);
                                                //     // echo $i.' <br/>';
                                                ...
                                                if($i == 36){
                                                    $scheduled = explode(' ',trim($trelement->nodeValue));
                                                    if(trim($scheduled[0]) == 'SCHEDULED'){
                                                        $scheduled_order = true;
                                                    }
                                                }
                                                if($scheduled_order === false){
                                                    if (($i == 44 || $i == 22)) {
                                                        $delivery_details = explode('  ', trim($trelement->nodeValue));
                                                    }
                                                    if (($i == 52 || $i == 75)) {
                				...
                				// if(count($order_details) == 0){
                    				...
                    				foreach ($items as $node) {
                                        ...
                                        //         }
                                        //         $str[] = $trelement->nodeValue;
                                        //         // echo $i."    " . $trelement->nodeValue."<br/>";
                                        //     }
                                        // }
                                        foreach ($tr as $trelement) { 
                                            if (count(explode('  ',trim($trelement->nodeValue))) > 1) {
                                                if($table_count == '8'){
                                                    if($scheduled_order && ($i == 4)){
                                                        $order_details = explode('  ', trim($trelement->nodeValue));
                                                    }
                                                    if($i == 5 && $scheduled_order === false){ 
                                                        $order_details = explode('  ', trim($trelement->nodeValue));
                                                    } 
                                                }
                                                // if($table_count == '7'){
                                                //     if($scheduled_order && ($i == 5)){
                                                //         $order_details = explode('  ', trim($trelement->nodeValue));
                                                //     } 
                                                // }else{ 
                                                //     if ($i == 4 && $scheduled_order === false) {
                                                //         $order_details = explode('  ', trim($trelement->nodeValue));
                                                //         $order_details = $order_details[1];
                                                //     } 
                                                ...
                                                $str[] = $trelement->nodeValue;
                                                // echo $i."    " . $trelement->nodeValue." ".$table_count."<br/>";
                                            }
                                        }
        ...
        //                                $i++;
    				...
    				// 			print_r($data);
                                ...
                                foreach ($items as $node) {
                                    ...
                                    if(trim($str[0]) == "SCHEDULED ORDER:  CONFIRM NOW" && $table_count != 13){
...
//                                    print_r($order_item);
                                    ...
                                    if($table_count < 9 || $table_count > 12){
                                        if(trim($customer_details[0]) == 'Pickup by:'){
                                            $trans_type = 'pickup';
                                            $customer_name = trim($customer_details[1]);
                                        }else{
                                            $customer_name = explode(':',$customer_details[0]); 
                                            $customer_name = trim($customer_name[1]);
                                        }
                                    //     print_r($customer_details);
                                    // print_r($customer_name);continue;
                                        $client_info['name'] = $customer_name;
                                        $trans_type = 'delivery';
                                        if($table_count > 12){
                                            $delivery_details_date = $delivery_details[3];
                                        }else{
                                            $delivery_details_date = $delivery_details[1];
                                        }
                                    }else{
                                        $delivery_details_date = $delivery_details[2];
                                    }
                                    $delivery_date = '';
                                    $delivery_time = '';
                                    if(isset($delivery_details_date)){
                                        $delivery_date_time = strtotime($delivery_details_date);
                                        if(date('Y',$delivery_date_time) == '1969'){
                                            $date_time = explode(',',$delivery_details_date);
...
//                            }
	...
	public function actionFoodOrderDoordash(){ 
	   require ('/var/www/vhosts/orderdindin.com/httpdocs/vendor/autoload.php');
           require (Yii::app()->basePath.'/components/simple_html_dom.php');
            $client = new Google_Client();
            // $fromEmail = 'foodorders@omnitech.pro';
            $fromEmail = 'doordash@dindint.com';
            $client->useApplicationDefaultCredentials();
            $client->setSubject($fromEmail);
            $client->setApplicationName("Doordash");
            // $client->setDeveloperKey("AIzaSyAPlbyYvlQVw_pSJBkKJM7LCN6DrMgmPQY");
            // $client->setAuthConfig(Yii::app()->basePath.'/components/order-dindin-doordash-645ee42d1b21.json');
            // $client->setScopes(["https://mail.google.com/",
            //     "https://www.googleapis.com/auth/gmail.compose",
            //     "https://www.googleapis.com/auth/gmail.modify",
            //     "https://www.googleapis.com/auth/gmail.send",
                                    ...
                                    foreach ($tr as $trelement) {
                                        if (count(explode('  ',trim($trelement->nodeValue))) > 1) {
                                            ...
                                            if($table_count < '7'){
                                                ...
                                                if($scheduled_order === false){ 
                                                    if (($i == 44 || $i == 22) && count($delivery_details) == 0) {
                                                        $delivery_details = explode('  ', trim($trelement->nodeValue));
                                                        // print_r($delivery_details);
                                                    }
                                                    if (($i == 52 || $i == 75) && count($customer_details) == 0) {
                                                        $customer_details = explode('  ',$trelement->nodeValue); 
                                                        // print_r($customer_details);
                                                    }
                                                }

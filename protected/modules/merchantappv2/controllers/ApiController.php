...
//     }  //previous working function
    ...
    //this is the test in which one parameter missing and updated
     public function actionGetTodaysOrder()
    {    
        $Validator=new Validator;
		...
		if ($Validator->validate()){
			if ( $res=merchantApp::validateToken($this->data['mtid'],
				...
				AND 
				request_cancel='2'
			    ORDER BY viewed = '1' DESC, order_id DESC
				LIMIT 0,100
				";
    ...
    //Tets Function which i pick from dev because live was not working
    
//      public function actionGetTodaysOrder()   //working function
//     {    	
			    ...
			    $this->data['token'],$this->data['user_type'])){
					if ( $data=Yii::app()->functions->getOrder2($this->data['order_id'])){
						...
						if($data['confirmed'] == 0){
							...
							// echo ' '.date('G:i').' '.$delivery_time;
							// update the order id to confirmed and time in added
                            //
...
// dump($receipt);
					...
					}else { $this->msg=$this->t("order details not available");$this->code=3; }
	...
	//here creating the test function to fix doodash Api Issue
	public function actionUpdateOrderStatusTest(){
		...
		if ( $res=merchantApp::validateToken($this->data['mtid'],
			    $this->data['token'],$this->data['user_type'])){
					if ( $data=Yii::app()->functions->getOrder2($this->data['order_id'])){
					   ...
					   //print_r($data['delivery_contact']); exit('dcs');
				...
				// 			$data['confirmed'] = 0;
						...
						if($data['confirmed'] == 0){
                            ...
                            // print_r($merchant_info['contact_phone']); echo $data['trans_type']; exit('awdw');
                            //here overrinding the values
                            // $merchant_info['service'] = '8';
                            // $data['trans_type'] = 'delivery';
                            ...
                            if($merchant_info['service'] == 8 && $data['trans_type'] == 'delivery'){
                                ...
                                $doordash_time = date('G:i:s',$the_date);
                                $delivery_date = $doordash_date."T".$doordash_time.'Z';
                                $doordash_result = FunctionsV3::createDoordashDeliveryTest($delivery_date,$data,$merchant_info,$time_in_selected,$confirmed);
                                
                                // print_r($doordash_result); exit('dscs');
			...
			if ( $res=merchantApp::validateToken($this->data['mtid'],
			    $this->data['token'],$this->data['user_type'])){
					...
					if($this->data['end_time'] == 'today'){
						...
						// $store_s_date = date('Y-m-d G:i:s', strtotime(' -1 day'));
			...
			if ( $res=merchantApp::validateToken($this->data['mtid'],
			    ...
			    if ( $data=Yii::app()->functions->getOrder2($this->data['order_id'])){
			    	...
			    	//dump($data);
			    		...
			    		// for json_details formatting 

			    	 if($this->data['json']){
			    	     
			    	     $arras = json_decode($data['json_details'], true);
			    	     

    			    	if(!is_array($arr))
    			    	{
    			    	     $formated = array();
        			    	 foreach($arras as $akey => $arr){
    			    	         array_push($formated, $arr);
    			    	         
    			    	     }
    			    	    $data['json_details']= json_encode($formated);
			    	...
			    	array(
					  ...
					  $json_details,true,$data['order_id'],'api');
					  ...
					  if ( Yii::app()->functions->code==1){					  	  
                        ...
                        //   if($merchant_info['service'] == 8 && $data['trans_type'] == 'delivery'){
                        //       $data_raw['delivery_time'] = Yii::app()->functions->timeFormat($data['doordash_drive_pickup_time'],true);
                        //   }else{
                        //       $data_raw['delivery_time'] = Yii::app()->functions->timeFormat($data['delivery_time'],true);
                        //   }
						 ...
						 if($this->data['json']){
						   ...
						   if(!isset($data_raw['total']['cart_tip_value']))
						    ...
						    if(is_null($data_raw['auto_printed']))
						    {
						       $data_raw['auto_printed']=strval(0); 
						    }
						    
						  //  print_r($data_raw); exit('ko');
						 ...
						 //format according to android app  
						  ...
						  if(!isset($this->data['merchant_auto_print']) && $data_raw['viewed'] !=2 ){
    						  ...
    						  //if($data_raw['confirm_link_clicked'] != 1 && !is_null($data_raw['confirm_link'])){
    						  //  $clicked = $this->actionConfirmlink($data_raw['confirm_link']);    
    						  //}    					        						      
    ...
    public function actionConfirmlink($confirmlink){
        
        $curl = curl_init();
        
        curl_setopt_array($curl, array(
          CURLOPT_URL => $confirmlink,
			...
			if ( $res=merchantApp::validateToken($this->data['mtid'],
				...
				$this->data['user_type'],$this->data['mtid']
					...
					if ( $DbExt->updateData('{{mobile_device_merchant}}',$params,'id',$resp['id'])){
						...
						Yii::app()->functions->updateOption("printer_status",$this->data['printer_status']
						,$merchant_id); 
                                                
						Yii::app()->functions->updateOption("printer_ip",$this->data['printer_ip']
						,$merchant_id);  
                                                
						Yii::app()->functions->updateOption("printer_device_id",$this->data['printer_device_id']
						,$merchant_id);  
                                                
						Yii::app()->functions->updateOption("printer_timeout",$this->data['printer_timeout']
						,$merchant_id);         
						Yii::app()->functions->updateOption("number_prints",$this->data['number_prints']
						,$merchant_id);  
	...
	public function actionloadCancelOrder()
	{
		...
		if ($Validator->validate()){
			if ( $res=merchantApp::validateToken($this->data['mtid'],
				...
				} else $this->msg=$this->t("no cancel orders");
			} else {
				$this->code=3;
				$this->msg=$this->t("you session has expired or someone login with your account");
			}
		...
		if ($Validator->validate()){
		    if ( $res=merchantApp::validateToken($this->data['mtid'],
			    $this->data['token'],$this->data['user_type'])){
					...
					}else $this->msg = $this->t("It's not a valid order");
			    }else $this->msg = $this->t("Merchant id is incorrect");
		}else $this->msg = $this->t("you session has expired or someone login with your account");

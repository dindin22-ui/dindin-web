...
{	
	...
	public static function getUnOpenOrder($mtid='')
	{
		...
		$_in = '"pending","accepted","Accepted","Pending","initial_order","paid","Paid"';
		...
		$stmt="
    	      ...
    	      (
    	       ...
    	       and
    	       status NOT IN ('initial_order','rejected','Rejected')
    	      ) as total_order
    	      ...
    	      AND merchantapp_viewed<=0
    	      AND status NOT IN ('initial_order','rejected','Rejected')
    	      AND request_cancel='2'
    	      ORDER BY date_created DESC
    	";	
		if(isset($_GET['debug'])){

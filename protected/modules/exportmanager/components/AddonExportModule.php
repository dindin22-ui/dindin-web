				...
				foreach ($data as $val) {					
					...
					if (is_array($val['item']) && count($val['item'])>=1){
						foreach ($val['item'] as $params) {
							...
							if (isset($params['require_addon'])){
							    $require_addon = json_decode( $params['require_addon'] , true );
								$new_require_addon = array(); 
								foreach ($require_addon as $subcat_id => $subcategory_item) {									
								    $new_subcat_id=TempDBStructure::getNewID("tmp_subcategory",'subcat_id','subcat_id_new',$subcat_id);
								    $new_array = array();    
								    array_push($new_array, 2);
								    $new_require_addon[$new_subcat_id] = $new_array;    
								}
								    $params['require_addon']   = json_encode($new_require_addon, true) ;    
							}	
				...
				foreach ($data as $val) {					
					...
					if (is_array($val['subcategory']) && count($val['subcategory'])>=1){
						foreach ($val['subcategory'] as $params) {
							...
							$params['merchant_id']=$new_mtid;
							$params['subcategory_name_trans']=!empty($params['subcategory_name_trans'])?$params['subcategory_name_trans']:'';							
							$params['subcategory_print_name']= $params['subcategory_print_name'];
							$params['subcategory_description_trans']=!empty($params['subcategory_description_trans'])?$params['subcategory_description_trans']:'';
							$DbExt->insertData("tmp_subcategory",$params);							
						}						
						//TempDBStructure::showTempData('tmp_subcategory');
				...
				// 		TempDBStructure::showTempData('tmp_size');						
					...
					if (is_array($val['item']) && count($val['item'])>=1){
						foreach ($val['item'] as $params) {
							...
							$new_price= array();
							if (self::isArray($price)){								
								foreach ($price as $size_ids=>$actual_price) {									
									$i=TempDBStructure::getNewID("tmp_size",'size_id','size_id_new',$size_ids);
									$new_price[$i]=$actual_price;
								}								
								$params['price']=json_encode($new_price);
							}

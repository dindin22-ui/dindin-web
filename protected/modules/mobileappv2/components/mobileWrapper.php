...
class mobileWrapper
{
	...
	public static function milesAvailable()
    {
    	return array(
			'',mt('Select Radius'),
	    	2=>mt("2 Miles"),
...
/*	public static function prettySort($sort_by = '')
    ...
    public static function orderAllDetails($order_id='')
    {
    	$db = new DbExt();
    	$stmt="
    	SELECT 
    ...
    public static function itemsByOrder($order_id='')
    {
    	$db_ext=new DbExt;
    	$stmt="SELECT * FROM
    	{{order_details}}
    	...
    	if ( $res=$db_ext->rst($stmt)){
            ...
            foreach($res as $key => $ress){
                if($ress['addon']){
                    $res[$key]['addon'] = json_decode($ress['addon']);                    
                // print_r($res[$key]['addon']); exit('ijijij');  
                    
                }
                //     if($ress['addon']){
                //      $new_res = json_decode($ress['addon'],true);
                //      foreach($new_res as $key => $dd){
                         
                //          $values=array(
                //                 'addon_name'=>$dd['addon_name'],
                //                 'addon_category'=>$dd['addon_category'],
                //                 'addon_qty'=>$dd['addon_qty'],
                //                 'addon_price'=>$dd['addon_price'],
                //              );
                //             // print_r($values); exit('yy');
                //          $new_addon=[$dd['addon_category']=>$values];
                         
                //          array_push($new_addons, $new_addon);
                         
                //      }
                //      print_r($new_addons); exit('yy');
	...
	public static function getPushNotificationByDevice_uiid($device_uiid='', $check_active=true)
    {
    	if(empty($device_uiid)){
    		return false;
    	}

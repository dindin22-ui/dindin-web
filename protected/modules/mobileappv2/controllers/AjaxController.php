...
class AjaxController extends CController
{
    ...
    public function actionsavesettings()
    {
    	...
    	Yii::app()->functions->updateOptionAdmin('mobileapp2_api_splash_screen',
		isset($this->data['mobileapp2_api_splash_screen'])? trim($this->data['mobileapp2_api_splash_screen']) :''
		);
		
		//new fields.
		
		Yii::app()->functions->updateOptionAdmin('mobileapp2_api_app_version',
		isset($this->data['mobileapp2_api_app_version'])? trim($this->data['mobileapp2_api_app_version']) :''
		);
		
    	Yii::app()->functions->updateOptionAdmin('mobileapp2_api_screen_one',
		isset($this->data['mobileapp2_api_screen_one'])? trim($this->data['mobileapp2_api_screen_one']) :''
		);
		
		Yii::app()->functions->updateOptionAdmin('mobileapp2_api_android_app_version',
		isset($this->data['mobileapp2_api_android_app_version'])? trim($this->data['mobileapp2_api_android_app_version']) :''
		);
		
    	Yii::app()->functions->updateOptionAdmin('mobileapp2_api_push_notification',
		isset($this->data['mobileapp2_api_push_notification'])? trim($this->data['mobileapp2_api_push_notification']) :''
		);
		
    	Yii::app()->functions->updateOptionAdmin('mobileapp2_api_allow_geo_location',
		isset($this->data['mobileapp2_api_allow_geo_location'])? trim($this->data['mobileapp2_api_allow_geo_location']) :''
		);		
    ...
    public function actionsavesettings_app()
    {
		...
		Yii::app()->functions->updateOptionAdmin('mobileapp2_header_image',
		isset($this->data['mobileapp2_header_image'])?$this->data['mobileapp2_header_image']:''
		);
		...
		Yii::app()->functions->updateOptionAdmin('mobileapp2_report_order_email',
		isset($this->data['mobileapp2_report_order_email'])?$this->data['mobileapp2_report_order_email']:''
		);
    ...
    public function actionuploadFile()
    {
		...
		array_push($valid_extensions,"mp4");
		array_push($valid_extensions,"3gp");
	    ...
	    if (!$result) {
	    	 ...
	    	 if($_GET['id'] == 'header_image'){
	    	 	$remove_class='multi_remove_picture';
	    	 	$fields = '<input type="hidden" name="'.$_GET['field_name'].'" value="'.websiteUrl().'/upload/'.$new_filename.'" > ';
	    	    $img_ext = FunctionsV3::validImageExtension();
	    	    $display ='<image src="'. websiteUrl()."/upload/".$new_filename.'" style=" height: 150px; width: 150px;" id = "img_prev"> </image>'; 
	    	    
	    	    if (!in_array($ext, $img_ext)){
	    	        $display ='<iframe src="'. websiteUrl()."/upload/".$new_filename.'" style=" height: 150px; width: 150px;" id = "img_prev" title="Iframe Example"></iframe>';
	    	    }
	    	     
	    	     $html_preview='	    	 
        			  <a href="'. websiteUrl()."/upload/".$new_filename.'" target="_blank">'.$display.'
        			  <h5>View Header</h5></a>
        	    	 <div class="card '.$class_name.'" style="width: 10rem;">
        				<div class="card-body">
        				  <a href="javascript:;" data-id="'.$_GET['id'].'" 
        				  data-fieldname="'.$_GET['field_name'].'" 
        				  class="card-link '.$remove_class.'">'.mobileWrapper::t("Remove Header").'</a>
        				</div>
    ...
    public function actionuploadHeaderFile()
    {

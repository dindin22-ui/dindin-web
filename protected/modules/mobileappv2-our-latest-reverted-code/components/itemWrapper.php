...
// 	public static function getMenu($merchant_id='', $pagenumber=0, $pagelimit=10, $cat_id='') // the fake one
// 	{	
// 		$p = new CHtmlPurifier();
...
// 	}
	...
	public static function getMenu($merchant_id='', $pagenumber=0, $pagelimit=10, $cat_id='', $timezone='')
	{
...
// 	print_r($popularitemall);	    exit('mkjhg');
                    ...
                    if($mechant_res_cat){
                            ...
                            if($mechant_res_cat[0][$day] == 1){
                                ...
                                $end_date_data = date('Y:m:d h:i A', strtotime($val[$end_date]));
                                    if($today_formatted_data  < $start_date_data){
    ...
    public static function getMenuTest($merchant_id='', $pagenumber=0, $pagelimit=10, $cat_id='', $timezone='')
	{     
	    $tz = $timezone;
        $tz_obj = new DateTimeZone($tz);
        $today = new DateTime("now", $tz_obj);
		...
		if($merchant_id>0){
            ...
            if($pagenumber == 0){
                ...
                if( $popularItems &&  (count($popularItems) == 5) ){
                    ...
                    foreach($popularItems as $keypop => $popularItem){
                        ...
                        $popular['merchant_id'] = $popularItem['merchant_id'];
                        $popular['item_name'] = $popularItem['item_name'];
                        $popular['item_description'] = strip_tags($popularItem['item_description']);
                        if($popularItem['item_description'] == ''){
                            $popular['item_description'] = '';
                        }
                        
                        // print_r($popularItem['item_id']); exit('dsf');
                        ...
                        $popular['prices'] = $prices;
                        // print_r($popular['prices']);
                        
                        // print_r($popular['prices']); exit('prices');
		  ...
		  //  array_push($popularitemall['item'], $popular);
		  //  print_r($popularitemall); exit('popular');
    	   ...
    	   // popular_item end
			...
			if($res = $db->rst($stmt)){
				...
				// 	print_r($item_data); exit('ds');
					$data[]=$new_data;
				}

				// print_r($data); exit('dsf');
				// popular_item condition

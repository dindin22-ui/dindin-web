...
class ApiController extends CController {
    ...
    public function actioncreateAccount() {
        ...
        if ($Validator->validate()) {
            ...
            $this->msg = mobileWrapper::parseValidatorError($Validator->getError());
    ...
    public function actionresendCode() {
        ...
        if (!empty($customer_token)) {
            if ($res = mobileWrapper::getCustomerByToken($customer_token, false)) {
                ...
                switch ($verification_type) {
                    case "verification_email":
                        ...
                        if (!empty($email_address)) {
                            ...
                            $this->msg = $this->t("invalid email address");
                    ...
                    case "verification_mobile":
                        ...
                        if (!empty($contact_phone)) {
                            ...
                            $this->msg = $this->t("invalid mobile number");
                        break;
                ...
                $this->msg = $this->t("customer not found");
        } else
            $this->msg = $this->t("invalid customer token");
        $this->output();
    }
...
//                     // $exist = in_array($today_date, $res_holiday);
//                 // echo $today_date;
//                 // echo "u";
//                 // print_r($res_holiday); 
...
//                             $now_date_check = strtolower(date('l', strtotime( $now_date . " +1 days")));

//                             if($now_date_check == $valid_day){
//                                 $val['info']['timing_note'] = 'Opens at tomorrow '. $valid_opentime ; 
//                             }
//                             else{
//                                 $val['info']['timing_note'] = 'Opens at '.$valid_day.' '. $valid_opentime ; 
//                             }                            
...
//                               $val['info']['timing_note'] = 'Opens after '.$mint_diff  .' '. 'minutes';  ;
...
//                                 $val['info']['timing_note'] = 'This will open after '. $hour_diff .' '. 'hours';
...
//                                 $val['info']['timing_note'] = 'Opens after '. $hour_diff .':'.$mint_diff.' hours' ;
...
//                                 if($tomorrow = true){
//                                     $val['info']['timing_note'] = 'Opens tomorrow at '. $open_time_next_day ;        
//                                 }
//                                 else{
//                                     $val['info']['timing_note'] = 'Opens '.$now_date.' at '. $open_time_next_day ;        
//                                 }
//                             }
//                         }
...
// // 		print("the laat alone $lat , $lng"); exit("asdas");
...
//                     // print_r($val['open_status']); exit('iuhygtfrdr');
...
//                 // print_r($res_holiday); exit('sdfds');
...
//                 //     if (!in_array($today_date, $res_holiday)) {
...
//                         // print_r(strtotime($validate_hours[0])); exit('tinmxxx');
                       
                                         
//                 //Case when the customer is earlier then open time
                
//                 //  print_r($now_time); exit('sds');
                
//                         if(strtotime($now_time) <  strtotime($validate_hours[0])){
                            
//                             //  print_r(strtotime($now_time));
//                             //   print_r(strtotime($validate_hours[0]));
//                             //  exit('tinmeee');
//                             // exit('ii'); 
//                             // $now_time =  date('H:i A');
...
//                                 // print_r($open_time_next_day); exit('sdf');
...
//                 //     $merchant_id = 9;
//                 //      $val['info']['hours'] = FunctionsV3::getMerchantOpeningHoursArray($merchant_id);
//                 //     //  print_r( $val['info']['hours']); exit('dsc');
...
//                         //     echo $today_day; echo $today_date; echo "<pre>";  print_r($ondays); exit(' jiji');    
//                         // }     
...
//                             if($now_date_check == $valid_day){
//                                 $val['info']['timing_note'] = 'Opens tomorrow '. $valid_opentime ; 
//                             }
//                             else{
//                                 $val['info']['timing_note'] = 'Opens '.$valid_day.' '. $valid_opentime ; 
//                             }                            
...
//                             // print_r($now_hours); exit('tinmeee');
...
//                 // print_r($validate_hours[0]); exit('sds');
...
//                                 //change
//                                  $val['info']['timing_note'] = 'Opens at '. $validate_hours[0];
//                             }
...
//     }
    ...
    public function actioncustomerLogin() {
        ...
        if (!empty($user_mobile) && !empty($password)) {
            ...
            $this->msg = $this->t("either username or password is empty");
        $this->output();
    }
    ...
    // public function actionsearchByMerchantNameTest() {
    //     $merchant_name = isset($this->data['merchant_name']) ? $this->data['merchant_name'] : '';
    //     $cuisine_id = isset($this->data['cuisine_id']) ? $this->data['cuisine_id'] : '';
    ...
    //             //     $val['info']['hours'] = FunctionsV3::getMerchantOpeningHoursArray('9');
                    
    //             //     // print_r($val['info']['hours']); exit('dsfs');
                    
    //             // }
                
    //             // print_r($merchant_id); exit('dfds');
    ...
    //                 //  print_r($today_day); exit('sdfds');
                    
    //                 if( trim($days['day']) ==  trim($onday['day'] )){
    ...
    //             // print_r($today_day); exit('correct');
    ...
    public function actiongetItemByCategory() {
        ...
        if ($merchant_id > 0 && $cat_id > 0) {
            ...
            $this->msg = mt("invalid merchant id or category id");
        $this->output();
    }
    ...
    public function actionloadCart() {
        ...
        $search_mode = $search_resp['search_mode'];
        $location_mode = $search_resp['location_mode'];
        $auto_set_delivery_fee = 0;
        $resp_set_address = array();
        ...
        //   print_r($cart_details); exit('sdf');   coming
            ...
            if ($code == 1) {
                ...
                if ($merchant_info_raw = FunctionsV3::getMerchantById($this->merchant_id)) {
                    $merchant_info['merchant_id'] = $merchant_info_raw['merchant_id'];
    ...
    public function actionservicesList() {
        ...
        if (is_array($services) && count($services) >= 1) {
            ...
            $this->msg = $this->t("Services not available");
        $this->output();
    }
...
//                            $rsesp = StripeWrapper::retrievePaymentIntent($credentials['secret_key'], $payment_gateway_ref);
    ...
    public function actionsetAddressBook() {
        ...
        if ($addressbook_id > 0) {
            if ($res = Yii::app()->functions->getAddressBookByID($addressbook_id)) {
                ...
                $this->msg = $this->t("Address not available. please try again later");
        } else
            $this->msg = $this->t("Invalid address book id");
        $this->output();
    }
    ...
    public function actiongetOrderDetails() {
        ...
        if ($order_id > 0) {
            if ($res = mobileWrapper::orderAllDetails($order_id)) {
                ...
                $this->msg = $this->t("order not found");
        } else
            ...
            if ($website_review_type == 2) {
                ...
                if (!$res_review = FunctionsV3::getReviewByOrder($client_id, $order_id)) {
                    if ($db->insertData("{{review}}", $params)) {
                        ...
                        $this->msg = mt("ERROR. cannot insert data.");
                } else {
                    $id = $res_review['id'];
            ...
            if ($res = Yii::app()->functions->getOrderInfo($order_id)) {
                if ($res['client_id'] == $client_id) {
                    ...
                    if ($db->updateData("{{order}}", $params, 'order_id', $order_id)) {
                        ...
                        $this->msg = mt("ERROR: cannot update records.");
                } else
                    $this->msg = mt("Sorry but this order does not belong to you");
            } else
                $this->msg = mt("Order id not found");
        } else
            $this->msg = $this->t("invalid order id");
    ...
    public function actiongetOrderHistory() {
        ...
        if ($order_id > 0) {
            if ($res = mobileWrapper::orderHistory($order_id)) {
                ...
                // $order_info['city'] = $res[0]['city'];
                // $order_info['state'] = $res[0]['state'];
                // $order_info['location_name'] = $res[0]['location_name'];
        ...
        if ($order_id > 0) {
            ...
            if ($data = Yii::app()->functions->getOrder2($order_id)) {
                ...
                if ($json_details != false) {
                    ...
                    $this->msg = $this->t("Order not available to view. please try again later");
            } else
                $this->msg = $this->t("Order not available to view. please try again later");
        } else
            $this->msg = $this->t("invalid order id");
            ...
            if ($res = Yii::app()->functions->getOrderInfo($order_id)) {
                ...
                if ($resp = mobileWrapper::getCart($this->device_uiid)) {
                    if ($db->updateData("{{mobile2_cart}}", $params, 'device_uiid', $this->device_uiid)) {
                        ...
                        $this->msg = $this->t("Order not available to re-order. please try again later");
                } else {
    ...
    public function actionDeleteCreditCard() {
        ...
        if ($id > 0) {
            ...
            $this->msg = $this->t("invalid card id");
        $this->output();
    }
    ...
    public function actionDeleteAddressBook() {
        if ($client_id = $this->checkToken()) {
            ...
            if ($id > 0) {
                ...
                $this->code = 1;
                $this->msg = $this->t("Address has been deleted");
            } else
                $this->msg = $this->t("invalid id");
        }
        $this->output();
        ...
        if ($client_id = $this->checkToken()) {
            ...
            if ($id >= 1) {
                if ($res = Yii::app()->functions->getAddressBookByID($id)) {
                    ...
                    $this->msg = $this->t("Record not found. please try again later");
            } else
                $this->msg = $this->t("Invalid id");
        }
        $this->output();
    ...
    public function actionGetProfile() {
        ...
        if ($client_id = $this->checkToken()) {
            if ($res = mobileWrapper::getCustomerByToken($this->data['user_token'])) {
                ...
                $data['email_notification'] = $res['email_notification'];
                ...
                $this->msg = $this->t("Profile not found");
        }
        $this->output();
    }
    ...
    public function actionUpdateProfile() {
        ...
        if ($client_id = $this->checkToken()) {
            ...
            if ($db->updateData("{{client}}", $params, 'client_id', $client_id)) {
                ...
                $this->msg = $this->t("ERROR: cannot update records.");
        }
        $this->output();
    }
            ...
            if (isset($_FILES['file'])) {
                ...
                if ($upload_res) {
                    ...
                    if ($DbExt->updateData("{{client}}", $params, 'client_id', $client_id)) {
                        ...
                        $this->msg = self::t("ERROR: cannot update records.");
                } else
                    $this->msg = self::t("Cannot upload file");
            } else
                $this->msg = $this->t("Image is missing");
        }
    ...
    public function actionSaveBooking() {
        ...
        if (!Yii::app()->functions->bookedAvailable($merchant_id)) {
            if (!empty($fully_booked_msg)) {
                ...
                $this->msg = $this->t("Sorry we are fully booked for that day");
            $this->output();
        }
        ...
        if ($client_id = $this->checkToken()) {
            if ($this->merchant_id > 0) {
                ...
                if ($res == 1) {
                    ...
                    $this->msg = $this->t("successfully remove to your favorite list");
                $this->details = array(
            ...
            $this->msg = $this->t("You need to login to add this restaurant to your favorites");
        $this->output();
    }
    ...
    public function actionsearchMerchantFood() {
        ...
        if (!empty($search_string)) {
            ...
            if ($res = $db->rst($stmt)) {
                $data = array();
                foreach ($res as $val) {
                    // $val['distance'] = getOption($this->merchant_id, 'merchant_delivery_miles');
                    // $val['distance_unit'] = getOption($this->merchant_id, 'merchant_distance_type');
    ...
    public function actionDriverSignup() {
        ...
        if (FunctionsV3::hasModuleAddon('driver')) {
            ...
            if ($Validator->validate()) {
                ...
                if ($db->insertData("{{driver}}", $params)) {
                    ...
                    if ($status == "active") {
                        ...
                        $this->msg = t("Your request has been receive please wait while we validate your application");
            ...
            $this->msg = t("Failed. cannot find driver addon");
        $this->output();
    }
        ...
        if ($sms_balance >= 1) {
            ...
            if ($resp = Yii::app()->functions->sendSMS($contact_phone, $sms_msg)) {
                if ($resp['msg'] != "process") {
                    ...
                    $this->msg = $this->t("Sorry but we cannot send sms code this time");
            } else
                $this->msg = $this->t("Sorry but we cannot send sms code this time. please try again later");
        } else
            $this->msg = $this->t("Sorry but this merchant does not have enought sms credit to send sms");
    ...
    public function actionverifyOrderSMScode() {
        ...
        if ($resp = mobileWrapper::validateOrderSMSCode($sms_order_session, $order_sms_code)) {
            ...
            $this->msg = $this->t("Invalid SMS code");
    ...
    public function actionapplyVoucher() {
        ...
        if ($res) {
            ...
            $this->msg = $this->t("Invalid voucher code");
    ...
    public function actionapplyTips() {
        ...
        if ($tips > 0.0001) {
            ...
            if ($cart = mobileWrapper::getCartContent($this->device_uiid, $data)) {
                ...
                $this->msg = $this->t("cart not available");
        } else
            $this->msg = $this->t("Invalid tip");
        $this->output();
    }
        ...
        if ($client_id = $this->checkToken()) {
            ...
            if ($order_id > 0) {
                if ($order_details = Yii::app()->functions->getOrderInfo($order_id)) {
                    ...
                    $this->msg = $this->t("invalid order id not found");
            } else
                $this->msg = $this->t("invalid order id");
        }
        $this->output();
        ...
        if ($client_id = $this->checkToken()) {
            ...
            if (empty($error)) {
                ...
                if ($resp = AuthorizePayWrapper::Paynow($params, $client_id)) {
                    ...
                    $this->msg = AuthorizePayWrapper::$error;
            } else
                $this->msg = $error;
        }
        $this->output();
...
//       print_r(StripeWrapper::createToken());
    ...
    protected function doPayment($amount_to_pay,$order_id,$credentials,$payment_description,$client_info,$payment_intent,$token){
...
//                        $resp = StripeWrapper::retrievePaymentIntent($credentials['secret_key'], $payment_gateway_ref);
    ...
    public function actiongetPushSettings() {
        if (!empty($this->device_uiid)) {
            if ($res = mobileWrapper::getDeviceByUIID($this->device_uiid)) {
                ...
                $this->msg = $this->t("device uiid not found");
        } else
            $this->msg = $this->t("invalid device uiid");
        $this->output();
    }
    ...
    public function actionretrievePassword() {
        ...
        if (is_array($res) && count($res) >= 1) {
            ...
            $this->msg = $this->t("sorry the email or phone you have entered does not exist in our records");
    ...
    private function actiongeocode() {
        ...
        if (!empty($lat) && !empty($lng)) {
            ...
            $this->msg = mt("invalid latitude or longitude");
    ...
    public function actionclearRecentLocation() {
        if (mobileWrapper::clearRecentLocation($this->device_uiid)) {
            ...
            $this->msg = mt("failed deleting recent location");
        $this->output();
    }
    ...
    public function actionclearRecentSearches() {
        if (mobileWrapper::clearRecentSearches($this->device_uiid)) {
            ...
            $this->msg = mt("failed deleting recent searches");
        $this->output();
    }
    ...
    public function actioncheckRunTrackHistory() {
        ...
        if ($order_id > 0) {
            if ($res = mobileWrapper::getDriverTask($order_id)) {
                ...
                $this->msg = mt("invalid order id not found");
        } else
            $this->msg = mt("invalid order id");
        $this->output();
    }
        ...
        if ($client_id = $this->checkToken()) {
            ...
            if ($addressbook_id > 0) {
                if ($res = LocationWrapper::getAddressBookByID($addressbook_id, $client_id)) {
                    ...
                    $this->msg = mt("invalid address book not found");
            } else
                $this->msg = mt("invalid address book id");
        }
        $this->output();
    }
    ...
    public function actionrecheckLocation() {
        ...
        if ($distance) {
            ...
            $this->msg = mt("distance not available");
        $this->output();
    }
    ...
    public function actiongetActiveMerchantCategory() {
        ...
        $this->getGETData();
        if ($this->merchant_id > 0) {
            if ($res = itemWrapper::getMerchantCategory($this->merchant_id, $timezone)) {
                $this->code = 1;
                $this->msg = "ok";
                $this->details = array(
                    'data' => $res

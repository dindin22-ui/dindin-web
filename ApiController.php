    ...
    {    	
		...
		if ($Validator->validate()){
			if ( $res=merchantApp::validateToken($this->data['mtid'],
			    ...
			    $_in = '"pending","accepted","Accepted","Pending","paid","Paid","Grubhub","Doordash"';	
			    $DbExt=new DbExt;	
				$stmt="
				SELECT a.*,
				(
				...
				if ( $res=$DbExt->rst($stmt)){					
					...
					foreach ($res as $val) {
                                            if($val['delivery_service_type'] == 'grubhub' || $val['delivery_service_type'] == 'doordash'){
		...
		if ( $res=merchantApp::validateToken($this->data['mtid'],
			    $this->data['token'],$this->data['user_type'])){
					if ( $data=Yii::app()->functions->getOrder2($this->data['order_id'])){
						...
						$delivery_time = date('G:i', strtotime('+'.$time_in_selected.' minutes'));
			 ...
			 $show_time = true;  
			   if($data['delivery_asap']==1):
			    ...
			    if (isset($data['delivery_time']) && $show_time):
			    if ( !empty($data['delivery_time'])):       
				$print[]=array(
				 'label'=>$label_time,
				 'value'=>FunctionsV3::prettyTime($data['delivery_time'],true)
				);
				 endif;
			   endif;
			    ...
			    if ( $data=Yii::app()->functions->getOrder2($this->data['order_id'])){
			    	...
			    	array(
					  ...
					  if ( Yii::app()->functions->code==1){					  	  
						  ...
						  if($data['delivery_service_type'] == 'grubhub' || $val['delivery_service_type'] == 'doordash'){
						  ...
						  if($data['delivery_service_type'] == 'grubhub' || $data['delivery_service_type'] == 'doordash'){
